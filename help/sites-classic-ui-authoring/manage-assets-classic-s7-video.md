...
Il processo di codifica video predefinito si basa sull’utilizzo dell’integrazione basata su FFMPEG con i profili video. Pertanto, il [!UICONTROL Aggiorna risorsa DAM] il flusso di lavoro contiene i due passaggi seguenti, basati su ffmpeg:
...
1. Seleziona la **[!UICONTROL Video]** scheda.
   ...
   >[!NOTE]
   >
   >Il **[!UICONTROL Video]** La scheda non viene visualizzata se la pagina non dispone di una configurazione cloud. Consulta [Abilita Dynamic Media Classic per WCM](#enablingscene7forwcm).
...
1. Apri la finestra di dialogo per progettazione del componente video di base e cambia in **[!UICONTROL Profili]** scheda. Quindi elimina i profili predefiniti e aggiungi i nuovi profili video Dynamic Media Classic. L’ordine dell’elenco dei profili nella finestra di dialogo per progettazione definisce anche l’ordine dell’elemento sorgenti video durante il rendering.
1. Per i browser che non supportano HTML5, il componente Video consente di configurare un fallback flash. Apri la finestra di dialogo di progettazione dei componenti video e passa alla **[!UICONTROL Flash]** scheda. Configura le impostazioni del lettore Flash e assegna un profilo di fallback al lettore Flash.
