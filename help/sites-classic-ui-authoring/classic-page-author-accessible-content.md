...
seo-description: Le linee guida WCAG 2.0 sono costituite da un insieme di criteri di successo e linee guida che non dipendono dalla tecnologia in uso e hanno l’obiettivo di rendere i contenuti web accessibili e utilizzabili da persone con disabilità.
...
>[!CAUTION]
>
>Poiché l’interfaccia classica era obsoleta in AEM 6.4, il contenuto di questa pagina non è stato aggiornato per WCAG 2.1.
>
>Per informazioni dettagliate su AEM e WCAG 2.1, consultate le pagine seguenti:
>
...
AEM aggiunge un **Testo Alt** alle immagini per impostazione predefinita. Per l&#39;interfaccia classica, esistono due scenari diversi per la creazione dell&#39;attributo predefinito (anche se il valore predefinito potrebbe non essere sufficiente come alternativa e molto probabilmente dovrà essere modificato nella scheda delle proprietà dell&#39;immagine **Avanzate**):

* File:

   Un’immagine viene caricata dal disco rigido dell’utente. Se aggiungete un componente immagine a una pagina e quindi scegliete un’immagine dal disco rigido o da un’altra origine, il valore predefinito per **Alt Text** è `file`. Questa impostazione deve essere modificata nella scheda delle proprietà dell&#39;immagine **Avanzate**. Anche in questo caso, questo valore non viene visualizzato nel campo **Testo Alt**, ma quando il valore viene modificato, nel campo viene visualizzato il nuovo valore.

* Risorsa:

   Un’immagine viene aggiunta dall’archivio delle risorse digitali. Se trascinate un&#39;immagine dall&#39;archivio delle risorse digitali su una pagina Web, i valori **Title** e **Alt Text** per quell&#39;immagine saranno ricavati dai metadati dell&#39;immagine.

>[!NOTE]
>
>In entrambi gli scenari di cui sopra, il valore predefinito **Alt Text** non è visibile nella scheda **Proprietà immagine avanzate**. Per modificare il valore predefinito, è sufficiente immettere un nuovo valore nel campo **Testo Alt**.

>[!NOTE]
>
>Se l&#39;immagine è puramente decorativa (vedere [Creazione di buone alternative di testo](#creating-good-text-alternatives)), è possibile immettere uno spazio nel campo **Testo Alt** utilizzando la barra spaziatrice. In questo modo si crea un attributo `alt` vuoto, che richiede a un assistente vocale di ignorare l&#39;immagine.
...
* Grafici e diagrammi:
   ...
   >[!NOTE]
   >
   >Lo snippet di codice riportato sopra viene utilizzato solo per illustrare l’ordine. Si consiglia di utilizzare il componente **Immagine** (anziché il riferimento `img src` utilizzato sopra).
...
* **Testo con enfasi**:
   ...
   * Fare clic sull&#39;icona **B** (per &lt;strong>) o sull&#39;icona **I** (per &lt;em>) visualizzata nel pannello **Proprietà** (assicurarsi che HTML sia selezionato).
...
* **Utilizzare le tabelle**:
   ...
   * È consigliabile rimuovere eventuali valori predefiniti per **Larghezza**, **Altezza**, **Bordo**, **Margine celle**, **Spaziatura celle**. dato che queste proprietà possono essere impostate in un foglio di stile globale.
   ...
   È quindi possibile utilizzare le proprietà **Cella** per scegliere se la cella è una cella di dati o di intestazione e, se si tratta di una cella di intestazione, se si tratta di una riga o di una colonna o di entrambe:
...
È inoltre possibile modificare il titolo della pagina durante la modifica di una pagina, accessibile tramite la scheda **Barra laterale** - **Pagina** - **Proprietà pagina...**
