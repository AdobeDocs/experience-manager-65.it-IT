...
* Mappe, diagrammi, diagrammi:
    ...
    Per gli elementi grafici che forniscono dati spaziali (ad esempio  per descrivere le relazioni tra gli oggetti o un processo), assicurarsi che il messaggio chiave sia fornito in formato testo.  Per le mappe, fornire un equivalente testuale completo potrebbe non essere pratico, ma se la mappa è fornita come modo per aiutare le persone a trovare il modo di raggiungere una posizione particolare, il testo alternativo dell&#39;immagine mappa può indicare brevemente *Mappa di X*, quindi fornire indicazioni per tale posizione in un altro testo della pagina o attraverso il campo **Descrizione** nella scheda **Avanzate** del componente **Immagine** .
