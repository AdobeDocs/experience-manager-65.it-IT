...
>[!NOTE]
>
>Le configurazioni Blueprint ti consentono di inviare le modifiche al contenuto in Live Copy. Vedi [Live Copy: sorgente, blueprint e configurazioni blueprint](/help/sites-administering/msm.md#source-blueprints-and-blueprint-configurations).
...
* La pagina principale di ciascun ramo lingua ha una o più pagine secondarie. Quando crei una Live Copy, le pagine figlie vengono presentate come capitoli da includere nella Live Copy.
...
* Se crei la Live Copy da una pagina sorgente che **non** a cui si fa riferimento in una configurazione blueprint, il comando Rollout non sarà disponibile nelle pagine sorgente/blueprint.
...
1. Specifica il percorso di destinazione della Live Copy (apri la cartella o la pagina padre della Live Copy), quindi tocca o fai clic su **Successivo**.
...
1. (Facoltativo) Per specificare una o più configurazioni di rollout da utilizzare per la Live Copy, utilizza la funzione **Configurazioni di rollout** elenco a discesa per selezionarli; le configurazioni selezionate vengono visualizzate sotto il selettore a discesa.
...
Crea una Live Copy utilizzando una configurazione blueprint per creare un sito basato sul contenuto blueprint (sorgente). Quando crei una Live Copy da una configurazione blueprint, selezioni uno o più rami della lingua dell’origine blueprint da copiare, quindi selezioni i capitoli da copiare dai rami della lingua. Vedi [Creazione di una configurazione Blueprint](/help/sites-administering/msm-livecopy.md#creating-a-blueprint-configuration).

Se si omettono alcuni rami o capitoli della Live Copy, è possibile aggiungerli successivamente; vedere [Creazione di una Live Copy all’interno di una Live Copy (configurazione Blueprint)](#creating-a-live-copy-inside-a-live-copy-blueprint-configuration).
...
* **Configurazioni di rollout**: (Facoltativo) Seleziona una o più configurazioni di rollout da utilizzare per la sincronizzazione della Live Copy. Per impostazione predefinita, le configurazioni di rollout sono ereditate dalla blueprint; vedere [Specifica delle configurazioni di rollout da utilizzare](/help/sites-administering/msm-sync.md#specifying-the-rollout-configurations-to-use) per ulteriori dettagli.
...
1. Utilizza la **Lingue iniziali** selettore per specificare la lingua o le lingue del sito blueprint da utilizzare per la Live Copy.
...
1. Utilizza la **Capitoli iniziali** menu a discesa per selezionare le sezioni della blueprint da includere nella Live Copy. Anche in questo caso, tutti i capitoli disponibili sono inclusi per impostazione predefinita, ma possono essere rimossi.
...
Implementa una pagina blueprint per inviare le modifiche al contenuto in Live Copy. Un **Rollout** esegue le configurazioni di rollout che utilizzano il trigger [durante il rollout](/help/sites-administering/msm-sync.md#rollout-triggers).
...
I rollout vengono elaborati come processi asincroni e possono essere controllati nella [**Stato processi asincroni** dashboard](asynchronous-jobs.md#monitor-the-status-of-asynchronous-operations) a **Navigazione globale** -> **Strumenti** -> **Operazioni** -> **Processi**
...
>[!NOTE]
>
>L’elaborazione del rollout asincrono richiede AEM 6.5.3.0 o versioni successive. Nelle versioni precedenti, le pagine venivano elaborate immediatamente e in modo sincrono a meno che il **Rollout in background** opzione selezionata.
...
>[!NOTE]
>
>Se il componente è contrassegnato come contenitore, le azioni di annullamento e sospensione non si applicano ai suoi componenti figlio. Vedi anche [Best practice MSM](/help/sites-administering/msm-best-practices.md#components-and-container-synchronization) per ulteriori informazioni.
...
1. Apri le proprietà della pagina Live Copy utilizzando **Visualizza proprietà** comando **Sites** console o utilizzando **Informazioni pagina** sulla barra degli strumenti della pagina.
...
Sospendere l’ereditarietà di una Live Copy per una pagina è un’azione temporanea. Una volta sospesa, l’azione **Riprendi** diventa disponibile e consente di ripristinare la relazione live.
...
1. Apri [Panoramica di Live Copy](/help/sites-administering/msm-livecopy-overview.md#using-the-live-copy-overview) e seleziona una pagina Live Copy sospesa; verrà visualizzato come **EREDITARIETÀ ANNULLATA**.
...
* Passaggio a una Live Copy superficiale e successivo al passaggio al formato profondo:
   ...
   * Tutti gli elementi secondari della Live Copy (precedentemente poco profonda) sono trattati come se fossero stati creati manualmente e vengono quindi spostati utilizzando `[oldname]_msm_moved name`.
...
1. Apri le proprietà della pagina Live Copy utilizzando **Visualizza proprietà** opzione **Sites** console o **Informazioni pagina** sulla barra degli strumenti della pagina.
...
1. Apri **[Riferimenti](/help/sites-authoring/basic-handling.md#references)** e seleziona:
...
>
>Per rimuovere la relazione live con l&#39;opzione di reintegrarla in un secondo momento, puoi [annulla ereditarietà Live Copy](#suspending-inheritance-for-a-page) per la pagina.
...
* **Stacca su una pagina principale di una LiveCopy**
   ...
   Ulteriori modifiche alle pagine della blueprint (così com’era) **non** influisce sulla Live Copy (così come era).
...
* **Stacca su una pagina secondaria di una LiveCopy**
   ...
   *Tuttavia*, le sottopagine sono ancora soggette alla relazione live del ramo principale, pertanto un ulteriore rollout delle pagine blueprint consentirà a:
