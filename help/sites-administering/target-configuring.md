...
seo-description: Scopri come configurare manualmente l'integrazione con Adobe Target.
...
È possibile modificare le configurazioni della procedura guidata di consenso, oppure integrare manualmente Adobe Target senza ricorrere alla procedura guidata.

## Modifica delle configurazioni della procedura guidata di consenso {#modifying-the-opt-in-wizard-configurations}

La procedura guidata [di](/help/sites-administering/opt-in.md) consenso che [integra AEM con Adobe Target](/help/sites-administering/target.md) crea automaticamente una configurazione cloud di Target denominata Configurazione Target con provisioning. La procedura guidata crea inoltre un framework Target per la configurazione cloud denominato Provisioning Target Framework. Se necessario, potete modificare le proprietà della configurazione e della struttura del cloud.

Potete anche configurare Adobe Target per l&#39;utilizzo di Adobe Target come origine di reporting quando eseguite il targeting del contenuto configurando la configurazione A4T Analytics Cloud Configuration.
...
>[!NOTE]
>
>In AEM 6.3, potete selezionare il file della libreria Target, [AT.JS](https://marketing.adobe.com/resources/help/en_US/target/ov2/c_target-atjs-implementation.html), una nuova libreria di implementazione per Adobe Target progettata sia per le implementazioni Web tipiche che per le applicazioni a pagina singola.
>
...
>
>
Per ulteriori informazioni, consulta Note [sulla versione di](https://marketing.adobe.com/resources/help/en_US/target/rn/201604.html) Target.

### Proprietà Framework Target con provisioning {#provisioned-target-framework-properties}

Il framework Target con provisioning creato dalla procedura guidata di consenso è configurato per l&#39;invio di dati contestuali dall&#39;archivio dati profilo. Per impostazione predefinita, gli elementi relativi a età e genere dello store vengono inviati a Target. Probabilmente la soluzione richiede l&#39;invio di parametri aggiuntivi.
...
1. Nel menu a discesa Configurazione **cloud di Analytics** A4T, selezionate il framework.
   ...
   Fai clic su **OK**. Quando eseguite il targeting del contenuto con Adobe Target, potete [selezionare l&#39;origine](/help/sites-authoring/content-targeting-touch.md)del report.
...
>[!NOTE]
Il file della libreria Target, [AT.JS](https://marketing.adobe.com/resources/help/en_US/target/ov2/c_target-atjs-implementation.html), è una nuova libreria di implementazione per Adobe Target progettata sia per le implementazioni Web tipiche che per le applicazioni a pagina singola. Adobe consiglia di utilizzare AT.js invece di mbox.js come libreria client.
AT.js offre diversi miglioramenti rispetto alla libreria mbox.js:
* Tempi di caricamento delle pagine migliorati per le implementazioni Web
* Maggiore sicurezza
* Migliori opzioni di implementazione per le applicazioni a pagina singola
...
Per consentire ad AEM di interagire con Adobe Target, crea una configurazione cloud Target. Per creare la configurazione, dovete fornire il codice client Adobe Target e le credenziali utente.

Puoi creare la configurazione cloud di Target solo una volta, perché puoi associare la configurazione a più campagne AEM. Se disponete di diversi codici client Adobe Target, create una configurazione per ciascun codice client.
...
Utilizzate la procedura seguente per creare una configurazione cloud Target in AEM:
...
1. Passa ai servizi **** cloud tramite il logo **** AEM > **Strumenti** > **Distribuzione** > Servizi **** cloud. ([http://localhost:4502/libs/cq/core/content/tools/cloudservices.html](http://localhost:4502/libs/cq/core/content/tools/cloudservices.html))
   ...
   Viene visualizzata la pagina di panoramica di **Adobe Marketing Cloud** .

1. Nella sezione **Adobe Target** , fai clic su **Configura ora**.
1. Nella finestra di dialogo **Crea configurazione** :

   1. Assegna alla configurazione un **Titolo**.
   1. Selezionate il modello di configurazione **di** Adobe Target.
   1. Fai clic su **Crea**. 
...
1. Nella finestra di dialogo, inserite i valori per queste proprietà.
   ...
   * **Utilizza DTM per distribuire la libreria** client - Seleziona questa opzione per utilizzare AT.js o mbox.js da DTM o un altro sistema di gestione tag. Per utilizzare questa opzione è necessario [configurare l&#39;integrazione](/help/sites-administering/dtm.md) DTM. Adobe consiglia di utilizzare DTM invece di AEM per distribuire la libreria.
   ...
   * **AT.js** personalizzato: Lasciate vuoto se avete selezionato la casella Gestione dinamica dei tag o per utilizzare il file AT.js predefinito. In alternativa, caricate il vostro AT.js personalizzato. Viene visualizzato solo se avete selezionato AT.js.
   >[!NOTE]
   Per impostazione predefinita, quando scegliete di partecipare alla procedura guidata di configurazione di Adobe Target, viene attivato il targeting accurato.
...
Dopo aver configurato la configurazione cloud di Target, aggiungi un framework Target. Il framework identifica i parametri predefiniti inviati ad Adobe Target dai componenti Client Context [o](/help/sites-administering/client-context.md) ContextHub [](/help/sites-administering/contexthub-config.md) disponibili. Target utilizza i parametri per determinare i segmenti che si applicano al contesto corrente.
...
1. Seleziona le variabili che vuoi rendere visibili al sistema Adobe Target selezionando la casella di controllo **Condividi** nelle colonne appropriate.
...
Associate le attività [](/help/sites-authoring/activitylib.md) AEM alla configurazione cloud di Target in modo da poter eseguire il mirroring delle attività in [Adobe Target](https://marketing.adobe.com/resources/help/en_US/target/target/c_manage_content.html).
...
Dopo aver creato un framework Target in AEM, associate le pagine Web al framework. I componenti di destinazione nelle pagine inviano i dati definiti dal framework ad Adobe Target per il tracciamento. Consultate Targeting dei [contenuti](/help/sites-authoring/content-targeting-touch.md).
...
1. Tap/click **Edit**.
1. Tocca o fai clic su **Aggiungi configurazione** in Configurazioni **servizio** Cloud e seleziona **Adobe Target**.
...
* Quando si modifica l&#39;attività in Adobe Target, l&#39;URL punta a localhost. Per ovviare a questo problema, impostate l’esternalizzatore AEM sull’URL corretto.
