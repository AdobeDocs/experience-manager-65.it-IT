...
* **Supporto per il passaggio di XCI personalizzati:** Aggiunta del supporto per il passaggio di XCI personalizzati nei parametri dell&#39;applicazione cmdlet xmlformcmd. Questa funzionalità consente agli utenti di specificare file XCI personalizzati per il test, migliorando la flessibilità e il controllo sul processo di test. (LC-3923248)
...
* Gli assistenti vocali ricevono un messaggio parlato di `Loading` durante l&#39;aggiornamento della visualizzazione Sites. L’aggiornamento aggiunge un’area live di stato dedicata e scrive il messaggio in essa a livello di programmazione, che conferma l’avanzamento senza spostare lo stato attivo. (SITES-24481)
...
* Il campo Testo alternativo nella finestra di dialogo Collegamento ipertestuale dell’editor di testo ora espone un’etichetta programmatica. Gli assistenti vocali annunciano `Alternative text` per il campo e lo stato attivo si focalizza sul controllo con nome corretto. Questa correzione migliora la navigazione per gli utenti che utilizzano tastiera e riconoscimento vocale. (SITES-24675)
...
* Ora la finestra di dialogo Immagine annuncia il suo stato di caricamento attraverso un’area ARIA live. Gli assistenti vocali sentono un messaggio di `Loading, please wait` mentre viene visualizzato lo spinner. Inoltre, è un aggiornamento pronto al termine del contenuto, per consentire agli utenti di sapere quando possono interagire. (SITES-24697)
* La finestra di dialogo Selezione collegamento ora espone un’area attiva che annuncia i risultati della ricerca. Gli assistenti vocali sentono lo stato `results updated` dopo ogni ricerca senza spostare lo stato attivo, pertanto gli utenti ricevono una chiara conferma del completamento della ricerca. (SITES-24700)
...
* Modifica I filtri ora segnalano il loro stato come espanso o compresso. Il pulsante attiva `aria-expanded` in sincronia con il pannello dei filtri ed espone un singolo nome non crittografato (`Change filters`), rimuovendo l&#39;annuncio confuso di `filter?`. Gli utenti che usano un’utilità di lettura dello schermo possono prevedere il risultato dell’attivazione del controllo. (SITES-24713)
...
* L’azione di conferma nella finestra di dialogo Teaser viene visualizzata correttamente per gli assistenti vocali. Il controllo riporta `Confirm` non la descrizione dell&#39;icona, fornendo agli utenti di tastiera e utilità di lettura dello schermo indicazioni chiare. (SITES-25223)
* Il pulsante Aiuto ora mostra un nome chiaro e accessibile. Gli assistenti vocali annunciano `Help` invece di una descrizione dettagliata dell&#39;icona. Gli utenti conoscono l’azione e possono trovare assistenza più rapidamente. (SITES-25224)
...
* Gli assistenti vocali ora annunciano con precisione i controlli Annota e Chiudi annotazione nella barra degli strumenti Annotazione. NVDA non indica più `Preview button pressed,`, il che ha fuorviato gli autori e suggerito un&#39;azione errata. L’annuncio corrisponde al pulsante premuto e mantiene il flusso di lavoro pulito. (SITES-25234)
...
* Gli assistenti vocali annunciano le azioni corrette per i pulsanti **Annota** e **Chiudi annotazione**. NVDA non indica più `Preview button pressed`; annuncia il nome del pulsante in modo che gli utenti sappiano quando inizia o termina la modalità di annotazione. (SITES-25268)
...
* Il pulsante Modifica ora annuncia l’azione corretta agli assistenti vocali nella barra degli strumenti secondaria. L&#39;attivazione di questo comando consente di leggere `Edit` invece di `Preview button pressed,` non correlato, eliminando così confusione durante la navigazione da tastiera. (SITES-25320)
* Il cursore del carrello della barra degli strumenti Demografia ora mostra un nome accessibile corretto. Gli assistenti vocali annunciano `Cart total` e gli strumenti di input vocale possono eseguire il targeting del controllo per nome, migliorando la conformità a WCAG 4.1.2 (nome, ruolo, valore). (SITES-25322)
...
* AEM annuncia lo stato di caricamento nel modale Teaser. Il modale ora espone un messaggio di stato live durante il caricamento del contenuto, quindi NVDA e JAWS parlano `Loading, please wait.` Gli autori devono ricevere un feedback chiaro ed evitare di interagire con la finestra di dialogo prima che sia pronta. (SITES-25366)
...
* Il punto di riferimento della barra laterale Riferimenti ora annuncia il suo nome agli assistenti vocali. La regione espone un `aria-label` (`References rail`) univoco, che migliora la navigazione di riferimento e la distingue da altre regioni. (SITES-24973)
...
* Nell&#39;elenco a discesa Categorie non è presente un&#39;etichetta specifica, pertanto JAWS ha pronunciato un `images button menu.` generico. L&#39;aggiornamento assegna al controllo il nome `Categories` e ne definisce il ruolo. Gli utenti che usano un’utilità di lettura dello schermo sentono un’etichetta accurata e comprendono le scelte disponibili. (SITES-35226)
...
* Il campo multiplo utilizzato in un componente personalizzato mostrava pulsanti di icona senza etichetta e un ordine di tabulazione incoerente. JAWS/NVDA ha annunciato solo `button` o ignorato i controlli, bloccando il funzionamento della tastiera. L&#39;aggiornamento fornisce nomi descrittivi per Aggiungi, Rimuovi e Sposta, normalizza le tabulazioni e annuncia aggiornamenti dell&#39;elenco per soddisfare le aspettative ADA. (SITES-30660)
...
* Le proprietà dei frammenti esperienza ora localizzano la scheda **Riferimenti**. Le etichette e le intestazioni di colonna come `Page,` `Page path,` e `Variation title` vengono visualizzate nella lingua dell&#39;autore. Questa modifica rimuove le stringhe solo in inglese e mantiene la visualizzazione delle proprietà coerente per i team globali. (SITES-11203)
...
* Etichette tag personalizzate non riuscite nell&#39;Editor pagina e l&#39;interfaccia utente ha sempre visualizzato `Tags.` Il predicato ora valuta `fieldLabel` prima e `labelText` secondo, quindi applica il valore predefinito. Gli autori visualizzano l’etichetta che hanno impostato. (SITES-32278)
...
* Gli utenti hanno riscontrato problemi di accessibilità in DoR PDF forms, dove NVDA ha annunciato erroneamente `unavailable` per caselle di controllo, pulsanti di scelta e campi di testo. Il messaggio è stato spesso ripetuto, causando confusione per gli utenti di utilità di lettura dello schermo. (LC-3923201)
...
* Gli utenti visualizzavano una pagina primaria spagnola vuota quando in un pacchetto di dati XML (XDP) con più pagine primarie era selezionato solo il contenuto inglese. (LC-3923009)
* Gli utenti hanno osservato informazioni obsolete sull’anno del copyright in AEM Designer. Questo errore si è verificato nella casella popup all&#39;avvio, nella sezione `About` e nella sezione `Legal Notices`, visualizzando `2003-2024` invece di &quot;2003-2025&quot;. (LC-3923005)
* Gli utenti hanno rilevato una pagina PDF vuota durante l’utilizzo dell’impaginazione in AEM Forms Designer. Il problema si è verificato durante la selezione di `Top of the Next Page/Top of Page` per WireAdviceHeader, interrompendo il layout delle iterazioni di dati. (LC-3922997, LC-3922830)
* Gli utenti hanno riscontrato un problema che impediva la persistenza del valore digest file per la definizione schema XSD (Extensible Markup Language) nella versione a 64 bit di AEM Forms Designer. (LC-3922924)
...
* Si è verificato un errore con il colore di sfondo dell&#39;intestazione `Dati Richiedente` nel componente applicazione portatile (PAC) durante la ricezione del messaggio `path object not tagged.` (LC-3922912)
...
* Si è verificato un errore `Invalid URL` durante il tentativo di annotare risorse in AEM 6.5.22. (NPR-42684)
...
* L’icona Aiuto ora espone un’etichetta concisa alle tecnologie per l’accessibilità. JAWS legge `Help button` e non aggiunge più il testo `menu` estraneo. Questo aggiornamento rende il controllo conforme a WCAG 4.1.2 e semplifica l’utilizzo della tastiera e degli assistenti vocali. (GRANITE-55360)
...
* È stata corretta l’aggregazione stato-integrità in Panoramica del sistema. La visualizzazione ora si aggiorna dopo l’esecuzione dei singoli controlli e visualizza i conteggi corretti. Gli operatori visualizzano `OK` al superamento dei controlli di sicurezza e manutenzione, invece di un banner `2 errors` errato. (GRANITE-61482)
...
* È stato corretto il testo troncato nella descrizione nel pannello Immagine. La descrizione `Display caption as pop-up` viene eseguita completamente in tutte le lingue supportate, migliorando le indicazioni per gli autori non inglesi. (SITES-10490)
* Nella vista a colonne dell’amministrazione dei siti, le etichette localizzate troncate sono in francese e spagnolo. `End Time` e `Off Time` sono risultati troncati e non hanno mostrato alcun suggerimento. Adobe ha corretto le traduzioni e ripristinato la descrizione comando al passaggio del mouse, in modo che le etichette vengano lette completamente. (SITES-31318)
* Nella finestra di dialogo **Sposta** in Sites sono presenti chiavi i18n non elaborate anziché etichette leggibili. Elementi come `Referencing pages,` `Created on,` `Created by,` e `Path` sembravano illeggibili. La correzione aggancia la finestra di dialogo ai dizionari corretti e fornisce traduzioni, con un fallback in inglese. (SITES-30881)
