...
* **Limita risorse secondarie generate**: gli amministratori possono limitare il numero di risorse secondarie generate da [!DNL Experience Manager] per le risorse composte come PDF, PowerPoint, InDesign e i file Keynote. Vedi [Gestione risorse composte](/help/assets/managing-linked-subassets.md#generate-subassets).

  ![limita la generazione delle risorse secondarie](/help/assets/assets/sub-asset-limit.png)


* **Supporto Camera Raw**: è disponibile un nuovo pacchetto [!DNL Camera Raw] che supporta [!DNL Adobe Camera Raw] v10.4. Vedi [elaborare immagini utilizzando [!DNL Camera Raw]](/help/assets/camera-raw.md).
...
* È ora possibile utilizzare il servizio di conversione automatica dei moduli per [convertire PDF forms in moduli adattivi in francese, tedesco, spagnolo, italiano e portoghese](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/extending-the-default-meta-model.html?lang=it#language-specific-meta-model).
...
Di seguito è riportato l&#39;elenco delle correzioni fornite nella versione 6.5.10.0 di [!DNL Experience Manager].
...
* Dopo l&#39;aggiornamento al service pack 9, alcuni utenti non possono spostare cartelle in Experience Manager e visualizzare gli errori nei registri (SITES-1481).
...
* Il rendering delle pagine web non riesce quando i relativi collegamenti vengono modificati da bundle di filtri servlet personalizzati e così via. (NPR-36857) <!-- Note updated as per CQDOC-19203, April 07, 2022 -->
...
* In presenza di una struttura del sito con molte pagine secondarie, il caricamento delle pagine secondarie nella vista a colonne richiede più tempo in Experience Manager 6.5.8 rispetto ad Experience Manager 6.4.8.2 (CQ-4322766).
...
* Il valore della proprietà `jcr:title` non viene aggiornato nell&#39;istanza Publish dopo lo spostamento di una cartella. La ridenominazione e la ripubblicazione di una cartella all&#39;interno dell&#39;istanza di authoring non aggiornano il valore della proprietà `jcr:title` dello stesso nell&#39;istanza di pubblicazione (NPR-36369).
...
* L&#39;opzione [!UICONTROL Pubblicazione rapida] non è disponibile nella pagina dei risultati della ricerca (NPR-36904, CQ-4317748).
...
* Gli assistenti vocali ora comunicano il nome, la mansione e lo stato predefinito (selezionato o non selezionato) delle opzioni in [!UICONTROL All Assets] e [!UICONTROL All Collections] durante l&#39;esplorazione della pagina [!UICONTROL Publish] (CQ-4290705).

* Quando carichi un formato video non supportato (diverso da MP4) nella pagina [!UICONTROL Crea video interattivo], Experience Manager visualizza e annuncia messaggi di errore (CQ-4290700).
...
* Experience Manager smette di rispondere quando il nodo MongoDB principale passa a un altro nodo (NPR-36566).
...
* Quando si modifica un frammento di documento di testo, le nuove parole vengono visualizzate come tag HTML dopo il salvataggio del frammento (NPR-36837).

* Impossibile visualizzare le lettere salvate come bozze (NPR-36816).

* Quando modifichi un frammento di documento di testo e quindi visualizzi l’anteprima della lettera, AEM Forms visualizza la lingua dell’espressione nell’anteprima della lettera di HTML (CQ-4322331).
...
* Screen Reader non è in grado di leggere i dati dei campi mobili inseriti all’interno dell’etichetta di testo nella pagina master o nelle pagine del sottomodulo in un PDF dinamico (CQ-4321587).

**Servizi documentali**

* Quando si convertono i file XDP in file PDF e quindi si assembla il PDF risultante, le generazioni di PDF non riescono e viene visualizzato il seguente messaggio di errore:
...
UberJar per Experience Manager 6.5.10.0 è disponibile nell&#39;[archivio centrale Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.10/).
