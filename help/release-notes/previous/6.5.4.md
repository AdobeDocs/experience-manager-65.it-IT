---
title: '''[!DNL Adobe Experience Manager] 6.5.4 note sulla versione del service pack'
...
Adobe Experience Manager 6.5.4.0 è un aggiornamento importante che include nuove funzioni, miglioramenti richiesti dai clienti e prestazioni chiave, stabilità e sicurezza, introdotti dopo la data di disponibilità generale della versione 6.5 in **Aprile 2019**. Può essere installato sopra Adobe Experience Manager 6.5.
...
* Una nuova [Genera output stampabile](/help/forms/using/aem-forms-workflow-step-reference.md) step è ora disponibile per i flussi di lavoro di Adobe Experience Manager Forms.

![Genera output stampabile](/help/release-notes/assets/generate-print-output-step.gif)

* [Supporto di più colonne](/help/forms/using/resize-using-layout-mode.md) per la modalità di layout per moduli adattivi e comunicazioni interattive.
...
* È ora possibile sincronizzare sottostrutture di contenuto selettivo con *Dynamic Media - Modalità Scene7* invece di tutti gli elementi disponibili all’indirizzo `content/dam`.
...
* Le strutture di input o output SOAP e i dati complessi supportano ora la sostituzione dinamica dei gruppi.
...
* Riduzione del tempo necessario per caricare e inizializzare ContextHub all’interno di un sito (`contexthub.kernel.js`). Questo consente di caricare le pagine più rapidamente durante una visita al sito.
...
* È stato ridotto il tempo di caricamento delle voci in una [!DNL Sites] pagina con più di 200 Live Copy in **[!UICONTROL Panoramica Live Copy]**.

* È stata migliorata la gestione di URL incompleti o non validi. Tali URL possono rallentare l’Editor modelli.

## Sites {#sites-fixes}

* Quando l&#39;URL di una pagina Adobe Experience Manager Sites contiene due punti (`:`) o simbolo di percentuale (`%`), il browser smette di rispondere e i picchi di utilizzo della CPU (NPR-32369, NPR-31918).
...
* Impossibile sincronizzare la campagna in Adobe Campaign contenente il componente Design importer (NPR-31728) preconfigurato.
...
* La finestra di dialogo di configurazione della vista non funziona correttamente per la funzione del flusso di lavoro di personalizzazione della casella in entrata (NPR-32503 e NPR-32492). Per ulteriori informazioni su questa funzione, consulta [Casella in entrata](/help/sites-authoring/inbox.md).
...
* Impossibile generare la rappresentazione per le risorse con ${extension} (NPR-32294).
...
* Il file ZIP scaricato da DAM non può essere aperto utilizzando WinZip (NPR-32293).
...
* La pagina dei dettagli delle risorse PDF non mostra i pulsanti di azione, ad eccezione dei pulsanti A raccolta e Aggiungi rappresentazione, nell’Experience Manager in esecuzione in modalità di esecuzione Dynamic Media Scene7 (CQ-4286705).
...
* L’editor carosello non si apre in IE 11 in modalità ibrida Dynamic Media Experience Manager (CQ-4255590). **Ad Adobe, Dynamic Media - Clienti in modalità ibrida:** Adobe cesserà il supporto per Internet Explorer 11 in modalità Dynamic Media - Ibrido dopo maggio 2022.
...
* Assegnazione tag a Platform: la ricerca di tag digitando nei campi dei tag mostra i tag al di fuori dei limiti principali e non rispetta i `rootPath` proprietà dei campi tag (NPR-31895).
...
* La generazione dell&#39;URL della libreria di Launch si basa solo su `path` e `library_name` dall&#39;API di Launch e non è basato su `library_path` (NPR-31550).
...
Il canale di autorizzazione tra [!DNL Experience Manager Assets] e [!DNL Brand Portal] è stato modificato. Prima, [!DNL Brand Portal] è stato configurato nell’interfaccia classica tramite il gateway OAuth legacy, che utilizza lo scambio di token JWT per ottenere un token di accesso IMS per l’autorizzazione. [!DNL Experience Manager Assets] è ora configurato con [!DNL Brand Portal] da a [!DNL Adobe I/O], che fornisce un token IMS per l’autorizzazione del [!DNL Brand Portal] tenant.

I passaggi per configurare [!DNL Experience Manager Assets] con [!DNL Brand Portal] sono diversi a seconda del [!DNL Experience Manager] e se si sta effettuando la configurazione per la prima volta o se si stanno aggiornando le configurazioni esistenti. Consulta [Configurare Experience Manager Assets con Brand Portal](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/publish/configure-aem-assets-with-brand-portal.html) per i dettagli.
...
* Document Services: se il numero di transazioni in un server supera un limite specifico, la conversione da HTML a PDF non riesce e le impostazioni del tipo di file vengono rimosse da [!DNL Forms] (NPR-32204).
...
* Designer: se l’opzione relativa ai tag è abilitata, il bordo del sottomodulo scompare nell’output PDF generato (NPR-32547, NPR-31983, NPR-31950).
...
UberJar per Experience Manager 6.5.4.0 è disponibile nel [Archivio centrale Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.4/).
