...
Il nuovo protocollo DASH (Dynamic Adaptive Streaming over HTTP) supporta lo streaming con bitrate adattivo nella distribuzione di video Dynamic Media (con CMAF) [Formato comune dell&#39;applicazione multimediale] attivato).
...
* Una query GraphQL può utilizzare `damAssetLucene` indice invece di `fragments` indice. Di conseguenza, l’esecuzione delle query GraphQL potrebbe non riuscire o richiedere molto tempo.
