...
[!DNL Adobe Experience Manager] 6.5.1.0 è una versione importante che include correzioni per problemi di prestazioni, stabilità, sicurezza e miglioramenti fondamentali per i clienti, introdotti dopo la data di disponibilità generale di [!DNL Adobe Experience Manager] 6.5 del *aprile 2019.* Può essere installato sopra [!DNL Experience Manager] 6.5.
...
* Incluso supporto per la distribuzione di risorse video a 360 gradi in modalità [!DNL Dynamic Media]-Scene7.
...
* È stata aggiunta la possibilità di caricare e riprodurre 360 video sferici in modalità [!DNL Experience Manager] DM-Scene7. Hotfix per CQ-4265131
...
* Un problema con il lettore video Scene7 espanso a schermo intero. Hotfix per CQ-4266700
...
* La pubblicazione dei predefiniti immagine in [!DNL Brand Portal] non riesce e viene restituito il codice di errore 500. NPR-29510: Hotfix per CQ-4268659
...
* Avvisi nei registri quando si apre una pagina di modello SPA di cui è stato eseguito il rendering lato server. Hotfix per CQ-4270238
...
* La pubblicazione di [!DNL Experience Manager Assets] dalla cartella Author /content/dam/mac di [!DNL Experience Manager] in [!DNL Brand Portal] non funziona. NPR-29819: Hotfix per CQ-4271118
...
Gli elementi di rilievo per [!DNL Experience Manager Forms] 6.5.1.0 sono:

* Solo OSGi: è stato aggiunto un nuovo attributo `PAGECOUNT` nel servizio Output e Forms.

* Solo OSGI: è stato abilitato il supporto per la creazione di file PDF statici tramite il servizio Forms.
...
* I file CAD in PDF non riescono con la conversione JDK a 64 bit. NPR-29924, NPR-29925: Hotfix per CQ-4272113
...
* Quando si esegue un controllo di accesso facilitato completo sul PDF statico creato con [!DNL Experience Manager Forms Designer], il controllo della lingua primaria non riesce perché manca l&#39;attributo della lingua. Hotfix per CQ-4272923, CQ-4271002
...
* La verifica delle firme in un PDF tramite PDF Advanced Electronic Signatures (PAdES) genera InvalidOperationException. NPR-29842: Hotfix per CQ-4244837
...
* PDF non riesce la conversione in PDF/A-1b con il campo Modulo non ha l&#39;aspetto dict. NPR-29940: Hotfix per CQ-4269618
...
* Solo OSGi: è stato abilitato il supporto per la creazione di file PDF statici tramite il servizio Forms. NPR-28572: Hotfix per CQ-4270869
...
[Ottieni il file](/help/release-notes/assets/6_5-bundle-list.txt)

Elenco dei pacchetti di contenuti inclusi in [!DNL Experience Manager] 6.5.1.0

[Ottieni il file](/help/release-notes/assets/6_5-content-package-list.txt)
