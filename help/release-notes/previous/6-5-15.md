---
title: Note sulla versione di [!DNL Adobe Experience Manager] 6.5.15.0
...
* In Experience Manager Collections, il pulsante di rimozione `X` non contiene alcun testo da annunciare per gli assistenti vocali. (ASSETS-6912)
* Quando si utilizza Color Contrast Analyzer (Analisi del contrasto dei colori) in Experience Manager, non vi è alcuna differenziazione di colore tra la data corrente e la data scelta nella selezione della data del widget del calendario. Manca almeno un rapporto di contrasto di 3:1 in contrasto con i colori adiacenti. (ASSETS-6911)
* In Experience Manager Files, quando si seleziona una delle opzioni dal pulsante di opzione [!UICONTROL Pianificazione] in Gestisci pubblicazione, il nome e lo stato delle opzioni del pulsante di opzione vengono annunciati dall&#39;assistente vocale. Tuttavia, l&#39;etichetta **Pianificazione** non è annunciata. (ASSETS-6908, ASSETS-6906)
* Testo alternativo mancante per l’icona Ordina. (ASSETS-6904)
* Nella pagina delle proprietà della risorsa, il nome del campo `Person` nelle etichette delle schede dell&#39;estensione IPTC non viene annunciato dagli assistenti vocali. L’assistente vocale annuncia solo il campo modificabile e attualmente vuoto, ma non il nome dell’etichetta. (ASSETS-6903, ASSETS-6848)
...
* In Experience Manager Collections, un campo vuoto nella scheda **Advanced** mostra un rapporto di contrasto non corretto tra il limite e i colori adiacenti. (ASSETS-6895)
...
* Durante la copia di una raccolta, nel pulsante di rimozione dell&#39;immagine di SVG `X` con role=&quot;img&quot; manca role=&quot;presentation&quot;. Di conseguenza, l’immagine viene esposta agli utenti di utilità di lettura dello schermo. (ASSETS-6890)
...
* I testi segnaposto, il percorso e il testo alternativo non presentano un rapporto di contrasto di almeno 4,5:1 rispetto al colore di sfondo. (ASSETS-6884, ASSETS-6865)
...
* Nelle raccolte Experience Manager, il campo [!UICONTROL E-mail] non è identificato come campo obbligatorio e non viene visualizzato alcun messaggio di errore se non si specifica un valore. (ASSETS-6877)
* Nei file Experience Manager non viene visualizzato alcun messaggio di errore nella schermata **Condivisione collegamenti** in `Add Email Address`. L’errore viene identificato solo in utilizzando un colore. (ASSETS-6876, ASSETS-6875)
...
* Le opzioni Ambito, Flussi di lavoro e Fuso orario non presentano un rapporto di contrasto di 4,5:1 rispetto al colore di sfondo. (ASSETS-6868)
...
* I campi di input di testo vuoti, disponibili nella scheda IPTC di Proprietà risorsa, non presentano un rapporto di contrasto di 3:1 rispetto ai colori adiacenti. (ASSETS-6854, ASSETS-6847)
...
* Durante il caricamento di un PDF in Experience Manager, l’utilizzo di memoria è in costante aumento. (ASSETS-16889)
...
* Quando si passa da Brand Portal a Experience Manager 6.5, il filtro predicati utente non visualizza i risultati appropriati quando si applica il filtro per la prima volta. (ASSETS-15932)
...
* AEM Forms Designer è ora disponibile nelle [impostazioni locali spagnole](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html). (LC-3920051)
...
* Quando si visualizza l’anteprima di un modulo adattivo come modulo di HTML, il relativo rendering non viene eseguito correttamente, poiché alcuni dei moduli secondari si sovrappongono al modulo principale. (NPR-39046)
...
* In un server AEM Form su OSGi, quando si utilizza l’API Document Service per certificare PDF, l’operazione non riesce e viene restituito un errore: com.adobe.fd.signatures.truststore.errors.exception.CredentialRetrievalException: AEM-DSS-311-003. (NPR-38855)
* Quando l&#39;utente tenta di utilizzare il servizio wrapper per il rendering delle lettere con AEM 6.3 Forms, si verifica l&#39;errore `java.lang.reflect.UndeclaredThrowableException`. (CQ-4347259)
...
* Quando si tenta di eseguire una patch di AEM Form su JEE dopo aver modificato la password predefinita dell&#39;utente amministratore, si verifica un&#39;eccezione `com.adobe.livecycle.lcm.core.LCMException[ALC-LCM-200-003]: Failed to whitelist the classes`. (CQ-4348277)
* In AEM Designer, i campi modulo senza didascalie vengono inseriti nelle celle di tabella, incluse le caselle di controllo.(LC-3920410)
...
* Il contenuto del testo non dispone di un rapporto di contrasto di 4,5:1. (SITES-8756, SITES-8885)
* Il testo del collegamento o del pulsante non presenta un rapporto di contrasto di 4,5:1 al passaggio del mouse o allo stato attivo. (SITES-8756, SITES-8885)
...
* Ordinamento degli ordini dei titoli nella vista a elenco, sia crescente che decrescente, come vengono ordinati i titoli con ordine A, C, B. (SITES-7585)
* È stato aggiunto il supporto per l’assegnazione tag per le varianti di frammenti di contenuto. (SITES-8168)
* È stato identificato e rimosso il codice specifico di Odino da Experience Manager 6.5 che non era necessario. (SITES-3574)
...
* Nelle attività **[!UICONTROL Copy]** o **[!UICONTROL Livecopy]**, le informazioni `cq:targetMetadata` venivano duplicate in modo errato. A causa di questa funzionalità due o più frammenti di esperienza in Experience Manager puntavano alla stessa offerta esportata in Target. (NPR-38970)
...
### Editor di pagine {#sites-pageeditor-6515}
...
* I flussi di lavoro eseguiti correttamente su Experience Manager 6.5.11 non venivano eseguiti in modo coerente il 6.5.13 di Experience Manager. (NPR-39023)
...
>[!IMPORTANT]
>
>Adobe consiglia di non rimuovere o disinstallare il pacchetto [!DNL Experience Manager] 6.5.15.0. Pertanto, prima di installare il pacchetto, è necessario creare un backup di `crx-repository` nel caso sia necessario ripristinarlo. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. La versione del bundle OSGi `org.apache.jackrabbit.oak-core` è 1.22.13 o successiva (utilizzare la console Web: `/system/console/bundles`).
...
* [Elenco dei bundle OSGi inclusi in Experience Manager 6.5.15.0](/help/release-notes/assets/65150_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
