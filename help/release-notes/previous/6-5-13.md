---
title: Note sulla versione di [!DNL Adobe Experience Manager] 6.5.13.0
...
| Data | 26 maggio 2022 |
| URL di download | [Distribuzione del software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.13.0.zip) |
...
Sono state introdotte le correzioni di bug, le funzionalità chiave e i miglioramenti seguenti in [!DNL Experience Manager] 6.5.13.0:
...
* Dopo l&#39;aggiornamento a [!DNL Experience Manager] 6.5 SP10, l&#39;editor dei metadati della risorsa non funziona correttamente. (CQ-4341359)
...
* Quando si cambia lingua in **[!UICONTROL Preferenza utente]**, l&#39;etichetta **[!UICONTROL Ordina per]**, il pulsante a discesa e altre opzioni all&#39;interno delle opzioni di ordinamento nella home page di Asset non vengono visualizzati nella lingua selezionata. (CQ-4339306)
* Quando si aggiunge una regola a un campo a discesa in **[!UICONTROL Schema metadati]**, l&#39;elenco **[!UICONTROL Dipendente da]** non riflette l&#39;etichetta del campo del menu a discesa. (ASSETS-9442)
* Il menu a discesa Metadati di Assets disabilitato non mantiene i valori. (ASSETS-8918)
* Quando si visualizza la risorsa utilizzando l&#39;opzione **[!UICONTROL Ulteriori dettagli]** nella visualizzazione **[!UICONTROL Colonna]**, vengono visualizzate annotazioni non corrette. (ASSETS-8851)
...
* Il visualizzatore dimensionale non funziona in Chrome 97. (CQ-4340456)
...
* Anteprima protetta basata su token per le risorse Dynamic Media negli autori AEM. (ASSETS-4995)
* Durante la creazione di un predefinito immagine per Dynamic Media in [!DNL Experience Manager], il massimo consentito è limitato a 2000x2000 pixel nell&#39;interfaccia utente. Quando il valore viene aumentato a 2001 pixel per larghezza o altezza, il pulsante **[!UICONTROL Salva]** viene disattivato. (ASSETS-5691)
...
* Accessibilità - Gli assistenti vocali non narrano il ruolo dell&#39;intestazione per il testo `Cannot save this set` nell&#39;avviso `Cannot save this set`, mentre navigano utilizzando il tasto di scelta rapida dell&#39;intestazione `H` e il tasto Freccia giù. (ASSETS-5569)
...
* Dopo l’aggiunta di un componente Dynamic Media su un sito e dopo la pubblicazione della pagina, la nuova risorsa Dynamic Media aggiunta non è visibile nella pagina pubblicata né è visualizzabile nella pagina di anteprima. Questo problema si verificava per entrambi i tipi di risorse immagine e video. (ASSETS-9467)

## Commerce {#commerce-6513}

* &quot;Everyone&quot; ha jcr:write in `/content/usergenerated/etc/commerce/smartlists`. (NPR-35230)
...
* Impossibile scaricare un pacchetto se il nome del pacchetto contiene il carattere speciale `+`. (NPR-38102)
...
* La codifica del codice a barre PDF417 prodotta da [!DNL Experience Manager] Forms non è valida per un gruppo di pulsanti di scelta. (NPR-38525)
...
* Dopo l&#39;applicazione di [!DNL Experience Manager] 6.5 Service Pack 11, si verifica la sovrapposizione del testo in Adaptive Forms. (NPR-38376, CQ-4342472)
...
* Quando si utilizza un flusso di lavoro [!DNL Experience Manager] per inviare un&#39;e-mail in lingua ebraica, l&#39;e-mail ricevuta alla fine dell&#39;utente contiene punti interrogativi (??) al posto del testo in lingua ebraica (NPR-38296).
* Gli utenti vengono disconnessi in modo casuale da [!DNL Experience Manager] istanze Publish e un modulo adattivo non riesce a inviare. Il problema viene visualizzato nelle istanze [!DNL Experience Manager] che utilizzano Dispatcher. (NPR-38285)
* Quando si utilizza l’opzione getFormDataString in una regola di Adobe Launch per acquisire i dati del modulo adattivo, l’opzione non restituisce dati di Adaptive Forms. (NPR-38283)
...
* Quando si richiama un metodo del servizio SOAP end-point di AEM Forms JEE Service facendo clic su un pulsante configurato come regola di modulo adattivo, il servizio SOAP non riesce con l’eccezione seguente:
...
* Il componente AEM Forms incorporato nella pagina SITES utilizza l’attributo width con valore in % e non valido in base alla convalida W3C HTML. Errore di analisi non valido durante la convalida di HTML. (NPR-38124)
* Gli elementi dei pulsanti di scelta e delle caselle di controllo per la maggior parte dei temi predefiniti nei moduli adattivi non fanno parte dell’ordine di tabulazione (NPR-38108)
...
* Il rendering di una maschera con la proprietà Prepara elaborazione dati impostata non riesce in HTML Workspace. (CQ-4343294)
...
* Impossibile importare un file di Microsoft® Word in Forms Designer. L&#39;utente rileva l&#39;errore `Word (version XP or onwards) could not be found on the machine`. (CQ-4342146)
...
* L&#39;utente non è in grado di aggiornare alcuna configurazione nell&#39;interfaccia utente di gestione degli endpoint e ha riscontrato l&#39;errore ` Uncaught ReferenceError: updateEndpoint_required is not defined`. (CQ-4331523)
...
* La versione di Oak per 6.5.13.0 è stata aggiornata al 1.22.11. (NPR-38084)
...
* Aggiorna la dipendenza di `org.apache.httpcomponents.httpclient` in [!DNL Experience Manager] 6.5. (NPR-37999)
...
* [!DNL Experience Manager] Platform 6.5: Correzioni Di Denominazione Per La Lingua Cinese. (CQ-4308978)
* Passa a Launch per Omega Instrumentation. (NPR-38377)
* [!DNL Experience Manager] Platform 6.5: correzioni delle denominazioni locali cinesi. (CQ-4308978)
...
* Migliora il tempo di risposta all’apertura dell’Editor modello per frammenti di contenuto. I clienti che dispongono di numerosi frammenti in Assets potrebbero aver riscontrato degli errori all’apertura. (SITES-6284)
...
* L&#39;utilizzo di **[!UICONTROL Modifica]** dai risultati della ricerca potrebbe causare un errore `Not Found`. (NPR-37810)
...
* Abilitare il supporto per la prossima versione dei componenti core E-mail [https://github.com/adobe/aem-core-email-components](https://github.com/adobe/aem-core-email-components). (NPR-38445 e NPR-38204)
...
### Editor di pagine {#sites-pageeditor-6513}

* È stato migliorato l&#39;annullamento per i componenti che non dispongono di funzionalità inlineEditing o dropTarget in `cq:editConfig`. (NPR-38361)
...
* Il menu a discesa Sistema di stili potrebbe essere stato posizionato nella parte superiore della pagina invece che nel contesto del componente, per i componenti che utilizzano `cq:editConfig` &quot;afteredit: REFRESH_PAGE&quot;. Questo problema è ora risolto. (NPR-38384)
...
### Protezione {#sites-security-6513}
...
* A partire dal 6.5.13, non è più necessario creare il nodo del componente contenitore in JCR prima di apportare modifiche tramite l’Editor SPA. Viene creato un `virtual container` prima di salvarlo tramite SDK. (SITES-5762)
...
* Aggiorna il bundle `sling-javax.activation` con la correzione di SLING-8777. (NPR-38077)
...
* È stato modificato il comportamento di aggiunta di pagine ai progetti di traduzione a partire dal Service Pack 10. Il progetto di traduzione non contiene una nuova pagina creata [ex: test-page-women-2] nell&#39;elenco, se selezionata come pagina padre della nuova pagina creata [non direttamente]. (NPR-38256)
* Aggiungi la proprietà `cq:isTranslationLaunch` nei lanci creati dal progetto di traduzione. (NPR-38224)
* Launch viene creato per una pagina con un XF di riferimento contenente una risorsa. (NPR-38199)
* La memoria di traduzione dell&#39;aggiornamento [!DNL Experience Manager] non funziona. (NPR-38196)
...
* Aggiornamento a `favicon.ico` utilizzato in Experience Manager. (CQ-4315324)
* Quando ti trovi nella pagina iniziale > sezione Strumenti e fai clic sull&#39;icona [!DNL Experience Manager], viene visualizzata la schermata di navigazione [!DNL Experience Manager]. (NPR-38417)
...
* [!DNL Experience Manager] MCM (Campaign) 6.5: Correzioni dei nomi delle lingue cinesi. (CQ-4308973)
...
* [!DNL Experience Manager] 6.5.13.0 richiede [!DNL Experience Manager] 6.5. Per istruzioni dettagliate, consulta la [documentazione sull&#39;aggiornamento](/help/sites-deploying/upgrade.md).
...
* In una distribuzione con MongoDB e più istanze, installare [!DNL Experience Manager] 6.5.13.0 in una delle istanze di authoring utilizzando Gestione pacchetti.

>[!NOTE]
>
>Adobe sconsiglia di rimuovere o disinstallare il pacchetto 6.5.13.0 di [!DNL Experience Manager].
...
1. Scarica il service pack da [Distribuzione software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.13.0.zip).
...
Per installare automaticamente [!DNL Experience Manager] 6.5.13.0 è possibile utilizzare due metodi diversi.
...
1. Nella pagina delle informazioni sul prodotto (`/system/console/productinfo`) viene visualizzata la stringa di versione aggiornata `Adobe Experience Manager (6.5.13.0)` in [!UICONTROL Prodotti installati].
...
1. La versione del bundle OSGi `org.apache.jackrabbit.oak-core` è 1.22.11 o successiva (utilizzare la console Web: `/system/console/bundles`).
...
>[!NOTE]
>
>Ignora se non utilizzi [!DNL Experience Manager] Forms. Le correzioni in [!DNL Experience Manager] Forms vengono distribuite tramite un pacchetto aggiuntivo separato una settimana dopo il rilascio pianificato del Service Pack [!DNL Experience Manager].
...
1. Se si utilizzano lettere in Experience Manager 6.5 Forms, installare il [pacchetto di compatibilità AEMFD più recente](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html?lang=it#forms-updates).
...
UberJar per [!DNL Experience Manager] 6.5.13.0 è disponibile nel [repository centrale Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.13/)&#x200B;(https://).
...
* [Pacchetto 1.0.3 del Frammento di contenuto di AEM con indice GraphQL](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq650%2Ffeaturepack%2Fcfm-graphql-index-def-1.0.3.zip)
...
* [Elenco dei bundle OSGi inclusi in Experience Manager 6.5.13.0](/help/release-notes/assets/65130_bundles.txt)
...
* Contatta l’[Assistenza Clienti di Adobe](https://experienceleague.adobe.com/docs/customer-one/using/home.html?lang=it).
