...
In [!DNL Experience Manager] 6.5.13.0 sono stati introdotti le correzioni di bug, le funzionalità chiave e i miglioramenti seguenti:
...
* Quando si apre una raccolta avanzata salvata con il filtro di ricerca applicato come Publish, il filtro di ricerca si trasforma automaticamente in Non pubblicato. (CQ-4341191)
...
* &quot;Everyone&quot; ha jcr:write su `/content/usergenerated/etc/commerce/smartlists`. (NPR-35230)
* L’ordinamento locale dei prodotti Commerce non funziona più. (CQ-4343750)
* Publish Impossibile eseguire la ricerca rapida di un prodotto dalla pagina dei risultati della ricerca dopo la modifica delle proprietà. (CQ-4343318)
...
* Quando si richiama un metodo di servizio endpoint SOAP del servizio JEE di AEM Forms con un clic su un pulsante configurato come regola di modulo adattivo, il servizio SOAP non riesce con l’eccezione seguente:
...
* Quando si importa e si pubblica una lettera che include un nuovo file XDP, l’anteprima delle lettere non riesce nell’istanza di Publish. Tuttavia, se le lettere vengono importate e pubblicate una seconda volta utilizzando lo stesso file CMP, l’anteprima delle lettere viene eseguita correttamente. (CQ-4343599)
* Il rendering di una maschera con la proprietà Prepare data process impostata non riesce in HTML Workspace. (CQ-4343294)
...
* La versione di Oak per 6.5.13.0 è ora aggiornata al 1.22.11. (NPR-38084)
...
* A partire dal 6.5.13, non è più necessario creare il nodo del componente contenitore in JCR prima di apportare modifiche tramite l’editor SPA. Viene creato un `virtual container` prima di salvarlo tramite l&#39;SDK. (SITES-5762)
...
>[!NOTE]
>
>Adobe consiglia di non rimuovere o disinstallare il pacchetto [!DNL Experience Manager] 6.5.13.0.
...
1. Se utilizzi lettere in Forms Experience Manager 6.5, installa il [pacchetto di compatibilità AEMFD più recente](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html#forms-updates).
