...
| Data | venerdì 24 agosto 2023 <!-- UPDATE FOR EACH NEW RELEASE --> |
| URL di download | [Distribuzione software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.18.0.zip) <!-- UPDATE FOR EACH NEW RELEASE --> |
...
* Assets - Dai risultati della ricerca, ora puoi passare alla posizione della cartella che contiene una risorsa per eseguire varie attività di gestione delle risorse. (ASSETS-23182)
...
* ActiveMQ in AEM è obsoleto. ActiveMQ è stato utilizzato per la comunicazione tra due istanze AEM Publish. Adobe consiglia ai clienti di utilizzare ora un load balancer.
...
* L’implementazione GraphQL di Experience Manager 6.5 non era alla pari con master e mancavano diverse correzioni importanti. (SITES-13096)
...
#### Editor di pagine{#sites-pageeditor-6518}
...
* Gli stili aggiunti in Experience Manager 6.5, Service Pack 17, per i tag di ancoraggio e body causavano problemi CSS. I tag di ancoraggio non venivano visualizzati sottolineati in Author. (SITES-14261)
...
* Experience Manager visualizza messaggi di errore e di avviso durante il caricamento di una risorsa nell&#39;istanza AEM 6.5.17.0. (ASSETS-26232)
* Quando colleghi una risorsa da una cartella con accesso completo a una risorsa da una cartella con accesso in sola lettura, Experience Manager visualizza un messaggio di errore ma crea comunque una relazione parziale tra le due risorse. (ASSETS-25832)
* Assets connesso non funziona tra un’istanza AMS e un’istanza Cloud Service. (ASSETS-24930)
...
* Experience Manager visualizza una schermata vuota durante la modifica e l’annotazione di un’immagine nella vista a colonne. (ASSETS-24108)
...
* Experience Manager visualizza un messaggio di errore non localizzato quando si aggiunge un percorso JSON nello schema metadati. (ASSETS-21025)
...
* Quando si passa alle opzioni di modifica per una risorsa di tipo immagine utilizzando la tastiera, l&#39;opzione [!UICONTROL Avvia mappa] non viene riconosciuta e lo stato attivo della tastiera passa al pulsante Annulla. (ASSETS-17238)
...
* Sono stati corretti i predefiniti visualizzatore disabilitati visualizzati nel componente Dynamic Media. (ASSETS-22922)
* &quot;Image Server&quot; è stato rimosso dalla scheda Sicurezza delle impostazioni generali. (ASSETS-24618)
* È stato corretto un errore di caricamento delle risorse in Dynamic Media e StringIndexOutOfBoundsException. (ASSETS-25787)
* È stato aggiunto un asterisco visivo per il campo di modifica obbligatorio &quot;larghezza&quot; nella scheda &quot;Base&quot;. (ASSETS-25741)
* È stato corretto il download della rappresentazione di elementi multimediali dinamici Watermark. (ASSETS-26173)
...
* **Servizi documentali**
   ...
   * Se il server viene arrestato durante la generazione del documento di PDF, vengono generati errori di elaborazione dei processi di avvio post-server. L&#39;argomento -Dcom.adobe.livecycle.dsc.deferServiceStart=true deve essere aggiunto durante l&#39;avvio del server. (FORMS-9836)
   * Se un utente tenta di unire i PDF utilizzando il metodo AssemblerService.Invoke, l’assemblatore non riesce a eseguire l’operazione. (FORMS-9550)
   ...
   * Dopo l&#39;aggiornamento ad AEM Service Pack 6.5.14.0, si verifica il problema nel servizio assembler quando si utilizza un modello specifico. (FORMS-9445, FORMS-9408)
...
* **Moduli adattivi**
   ...
   * Se un modulo adattivo viene ricaricato dopo l&#39;aggiornamento ad AEM 6.5.16.0 Service Pack, il recupero dell&#39;allegato non riesce. (FORMS-8906)
...
* **Accessibilità**
   ...
   * Quando un assistente vocale legge un modulo di HTML, si verifica il problema durante la lettura del testo con crenatura (spaziatura). (FORMS-9258)
...
* Dopo l&#39;installazione di un Experience Manager Vanilla, Service Pack 17, vengono visualizzati errori in `stderr.log`. Le installazioni di Vanilla non devono generare errori. (CQ-4353637)
...
* Impossibile creare o accedere a, o entrambi, i nodi della cache di ContextHub su Experience Manager. (NPR-40515)
...
* Con AEM 6.5.15.0, tutti gli URL personalizzati, gli alias sling e la mappatura sling vengono interrotti dopo un riavvio di AEM. (NPR-40420)
...
* Come descritto nella [documentazione del browser di configurazione](https://experienceleague.adobe.com/docs/experience-manager-65/administering/introduction/configurations.html?lang=it#using-configuration-browser), _il nome diventa il nome del nodo nell&#39;archivio_. Tuttavia, nel browser configurazioni, il titolo della configurazione viene utilizzato per il percorso in CRXDE Lite e il nome della configurazione viene ignorato. (NPR-40607)
...
* [!DNL Experience Manager] 6.5.18.0 richiede [!DNL Experience Manager] 6.5. Per istruzioni dettagliate, consulta la [documentazione sull&#39;aggiornamento](/help/sites-deploying/upgrade.md). <!-- UPDATE FOR EACH NEW RELEASE -->
* Il download del service pack è disponibile in Adobe [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.18.0.zip).
* In una distribuzione con MongoDB e più istanze, installare [!DNL Experience Manager] 6.5.18.0 in una delle istanze di authoring utilizzando Gestione pacchetti.<!-- UPDATE FOR EACH NEW RELEASE -->

>[!IMPORTANT]
>
> Adobe consiglia di non rimuovere o disinstallare il pacchetto [!DNL Experience Manager] 6.5.18.0. Pertanto, prima di installare il pacchetto, è necessario creare un backup di `crx-repository` nel caso sia necessario eseguirne il rollback. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. Scarica il service pack da [Distribuzione software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.18.0.zip). <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!NOTE]
>
>Experience Manager 6.5.18.0 non supporta l&#39;installazione di Bootstrap. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. Nella pagina delle informazioni sul prodotto (`/system/console/productinfo`) viene visualizzata la stringa di versione aggiornata `Adobe Experience Manager (6.5.18.0)` in [!UICONTROL Prodotti installati]. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. La versione del bundle OSGi `org.apache.jackrabbit.oak-core` è 1.22.16 o successiva (utilizzare la console Web: `/system/console/bundles`). <!-- NPR-41010 for 6.5.18.0 --> <!-- OAK Oak oak VERSION -MAY- NEED TO BE UPDATED FOR EACH NEW RELEASE -->
...
* **La pubblicazione della pagina non funziona nell&#39;Editor pagina dopo l&#39;aggiornamento a Service Pack 18 (6.5.18.0)**
  ...
  Dopo aver aggiornato un&#39;istanza di AEM 6.5.0.0—6.5.17.0 a AEM 6.5.18.0, quando si fa clic su **Pubblica pagina** nell&#39;Editor pagina, si viene reindirizzati a un URL inesistente.
...
* Dopo l&#39;aggiornamento all&#39;ambiente di installazione completa JBoss Turnkey di AEM Forms 6.5.18.0 in Windows Server 2022, durante la compilazione del codice dell&#39;applicazione client di output tramite Java 11, potrebbe verificarsi il seguente errore di compilazione:
...
* Dopo aver installato il programma di installazione completo di AEM Service Pack 6.5.18.0, la distribuzione EAR non riesce in JEE utilizzando JBoss® Turnkey.
...
* L’anteprima a comparsa non funziona per il componente Firma scarabocchio in AEM 6.5 Forms Service Pack 18. (FORMS-12073) Per questo problema è disponibile un hotfix. Per scaricare e installare l&#39;aggiornamento rapido, vedere [Aggiornamenti rapidi di Adobe Experience Manager Forms](/help/release-notes/aem-forms-hotfix.md).
...
* Dopo l&#39;aggiornamento ad AEM Service Pack 18, non è possibile modificare le lettere di comunicazione interattive. (FORMS-10578) Per questo problema è disponibile un hotfix. Per scaricare e installare l&#39;aggiornamento rapido, vedere [Aggiornamenti rapidi di Adobe Experience Manager Forms](/help/release-notes/aem-forms-hotfix.md)
...
* [Elenco dei bundle OSGi inclusi in Experience Manager 6.5.18.0](/help/release-notes/assets/65180_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Elenco dei pacchetti di contenuti inclusi in Experience Manager 6.5.18.0](/help/release-notes/assets/65180_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
