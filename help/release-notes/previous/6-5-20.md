...
| URL di download | [Distribuzione software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.20.0.zip) <!-- UPDATE FOR EACH NEW RELEASE --> |

## Cosa è incluso in [!DNL Experience Manager] 6.5.20.0 {#what-is-included-in-aem-6520}

[!DNL Experience Manager] 6.5.20.0 include nuove funzionalità, miglioramenti chiave richiesti dai clienti, correzioni di bug e miglioramenti a livello di prestazioni, stabilità e sicurezza rilasciati dopo la disponibilità iniziale della versione 6.5 di aprile 2019. [Installa il service pack](#install) in [!DNL Experience Manager] 6.5.
...
* **Reporting delle transazioni in AEM Forms su JEE**: è stata introdotta la funzionalità di reporting delle transazioni per AEM Forms su JEE, che consente la registrazione completa delle transazioni dei documenti come Conversioni, Rappresentazioni e Invii. Questo miglioramento aumenta l’efficienza e facilita una migliore conservazione dei documenti. La funzione è disabilitata per impostazione predefinita. Puoi abilitarlo dall’interfaccia utente di amministrazione.
...
* Il campo `Workflow Title` è contrassegnato con `*` come obbligatorio, ma non c&#39;è alcuna convalida. (SITES-16491)
...
* Le cartelle di configurazione nidificate non erano più supportate e le cartelle dei modelli di frammenti di contenuto non erano più visibili dopo l’aggiornamento ad AEM 6.5.18 o AEM 6.5.19. (SITES-18110)
* Alcune sottocartelle non sono in grado di scegliere da modelli di frammenti di contenuto ereditati. Deve supportare le cartelle senza una proprietà `jcr:content`, anche se le cartelle DAM create tramite l&#39;interfaccia utente dispongono di tale nodo. (SITES-17943)
...
* Durante l&#39;esecuzione di una query GraphQL per [filtrare i risultati](/help/sites-developing/headless/graphql-api/graphql-api-content-fragments.md#filtering) utilizzando variabili facoltative, se per la variabile facoltativa è stato fornito un valore specifico **not**, la variabile verrà ignorata nella valutazione del filtro. (SITES-17051)
...
* L&#39;installazione del pacchetto dei componenti core di CIF su AEM causa la modifica del valore `:type` dei componenti esistenti. In seguito alla modifica, non verranno più riprodotte sulle pagine a cui sono state aggiunte. (SITES-17601)

#### Integrazione di Campaign{#sites-campaign-integration-6520}

* AEM utilizzava un inserisco nell&#39;elenco Consentiti di noto anche come `whitelist` a causa di un rapporto di vulnerabilità. Il inserisco nell&#39;elenco Consentiti di impediva ai clienti di utilizzare le funzionalità necessarie. (SITES-16822)
...
* Eccezione &quot;`Is not modifiable`&quot; durante il rollout del componente. In particolare, si verifica un&#39;eccezione `org.apache.sling.servlets.post.impl.operations.ModifyOperation` durante l&#39;elaborazione della risposta. (SITES-18809)
...
* Il pulsante Rollout MSM dalla pagina padre alla pagina figlio è interrotto nell&#39;interfaccia utente grafica touch. Se selezionato, viene visualizzato il seguente errore: `Uncaught TypeError: _g.shared is undefined`. (SITES-16991)

#### Editor di pagine{#sites-pageeditor-6520}
...
* Quando un PDF viene spostato in un&#39;altra posizione, l&#39;opzione **[!UICONTROL Visualizza pagina]** scompare. (ASSETS-30538)
...
* Quando una risorsa viene caricata in AEM, viene attivato il flusso di lavoro `Update_asset`. Tuttavia, il flusso di lavoro non viene mai completato. Il flusso di lavoro viene completato solo fino al passaggio di caricamento del prodotto. Il passaggio successivo è il caricamento batch di Scene7, ma tale processo non viene richiamato in AEM. (ASSETS-30443)
* Hai bisogno di un modo migliore per gestire correttamente i video non Dynamic Media nel componente Dynamic Media. Questo problema causava un&#39;eccezione durante la creazione dell&#39;istanza di `dynamicmedia_sly.js`. (ASSETS-31301)
* L’anteprima funziona per tutte le risorse, i set di video adattivi e i video. Tuttavia, genera un errore 403 per i file `.m3u8` (che, tra l&#39;altro, funzionano ancora tramite collegamenti pubblici). (ASSETS-31882)
* Lo stato `scene7SmartCropProcessingStatus` è stato corretto. Metadati video di ritaglio avanzato utilizzati per mostrare l’errore anche in caso di esito positivo. (ASSETS-31255)
...
* Quando un utente aggiunge caratteri a un modulo adattivo su un dispositivo Android™, può digitare più del numero massimo definito di caratteri nel campo Testo su focus out, sui dispositivi Android™. Tuttavia, funziona quando un utente seleziona il tipo di input HTML5. (FORMS-12748)
...
* Quando un utente esegue l&#39;aggiornamento a AEM Form Service Pack 6.5.19.0, dopo aver aggiornato una nuova lingua nel dizionario esistente non viene unita alle righe &quot;guideContainer&quot; per aggiungere una lingua a un modulo. (FORMS-12947)

* In AEM Forms Service Pack 6.5.19.0, l&#39;operazione del servizio Web richiamata in Java™ 11 non riesce e viene restituito l&#39;errore (FORMS-12329):
...
* Quando la modalità FIPS è abilitata in AEM Forms Service Pack 6.5.18.0, la creazione di un utente nel DOM predefinito non riesce e viene visualizzato l&#39;errore (FORMS-11857):
  `com.adobe.idp.cx.a: error seeding random number generator`

* Quando un utente seleziona i font in ADMINUI nel percorso `Home>Services>PDF Generator>Adobe PDF Settings`, l&#39;opzione non viene selezionata. Inoltre, in un profilo standard o personalizzato, la casella di riepilogo di Tipi di carattere disponibili è vuota. Di conseguenza, non è possibile personalizzare il sottoelenco di **Incorpora sempre** o **Non incorporare mai**. L’utente non è in grado di configurare il font per i PDF con PDF Generator. I registri non mostrano alcun messaggio di errore rilevante. (FORMS-12095)

* In AEM Forms Service Pack 6.5.18.0, l&#39;utente non è in grado di creare le impostazioni di protezione, non visualizza alcun errore o log del server, ma sullo schermo viene visualizzato un messaggio di errore popup. (FORMS-12212)

* Quando un utente in AEM Forms Service Pack 6.5.18.0 invia un modulo adattivo nel flusso di lavoro JEE, l’allegato nel modulo adattivo non viene inviato al processo JEE, causando un errore dell’applicazione. (FORMS-12232, FORMS-12228)
...
* In AEM Forms 6.5.18.0, quando un modulo adattivo viene pubblicato, tutte le sue dipendenze, inclusi i criteri, vengono ripubblicate, anche se non sono state apportate modifiche. (FORMS-10454)

* Quando un utente seleziona &quot;Microsoft® SharePoint&quot; durante l’esecuzione del gestore della configurazione in AEM Forms 6.5.19.1 con JBoss® Turnkey setup, l’installazione di LiveCycle JBoss® EAR non riesce e viene visualizzato il seguente errore: (FORMS-12463)
...
* Quando un utente esegue l&#39;aggiornamento a AEM Forms Service Pack 6.5.18.0, a causa di una gestione delle eccezioni mancante, gli XDP passati attraverso il servizio di output con l&#39;opzione PDF con tag abilitata non riescono. (LC-3921757)
...
* In AEM Forms Designer installato tramite Workbench, le informazioni sulla versione non sono esplicite in `Control Panel/Programs/Programs and Features`. (LC-3921976)

<!--* When a user creates an XDP on AEM Forms Designer, the user is not able to add the custom Caption Tag. (LC-3921246)-->

* Quando un utente crea un XDP su AEM Forms Designer, nell’output PDF il tag Button Form non è nidificato nel tag paragrafo principale (p-tag). (LC-3921719)

* Quando un utente crea un XDP su AEM Forms Designer, nell’output di PDF quando l’utente naviga tra i tag del modulo, viene taggato anche l’oggetto di sfondo. (LC-3921687)
...
* Aria-label &quot;close&quot; non è localizzato in **[!UICONTROL Assets]** > **[!UICONTROL File]**, seleziona una cartella, quindi sulla barra degli strumenti seleziona **[!UICONTROL Proprietà]** > **[!UICONTROL Autorizzazioni]** scheda > nome membro. (NPR-41705)
* Descrizione troncata per il campo **[!UICONTROL Password archivio chiavi]** nella pagina Impostazione SSL per le impostazioni internazionali ENG, FRA, KOR, DEU e PTB. (NPR-41367)

#### Platform{#foundation-platform-6520}

* Si è verificato un problema durante l’integrazione di Campaign con AEM a causa del quale il servlet /api non restituiva lo schema corretto nel json href. Il motivo era che AEM non stava ricevendo l’intestazione X-Forward-Proto che forzava la richiesta a rispondere con uno schema HTTP invece che HTTPS. Pertanto, è necessario aggiungere la possibilità di attivare/disattivare la selezione dello schema in base a una configurazione OSGI. (GRANITE-48454)

#### Sling{#foundation-sling-6520}

* Il bundle `org.apache.sling.resourceMerger` 1.4.2 genera un&#39;eccezione da AEM 6.5, Service Pack 17 e versioni successive. La fusione di risorse Sling 1.4.4 deve essere inclusa in Service Pack 20. (NPR-41630)

#### Traduzione{#foundation-translation-6520}

* Dopo la distribuzione di AEM 6.5 Service Pack 18, si è verificato un problema con la scheda Filtri nell’Editor regole di traduzione. Quando si seleziona un contesto, facendo clic su Modifica > Salva, alla successiva apertura dello stesso contesto viene visualizzato un carattere di doppia virgoletta come HTML. In sostanza, le regole di traduzione non venivano salvate correttamente. (NPR-41624)
* Problemi relativi alle traduzioni di frammenti di contenuto, in cui le stringhe tradotte vengono rimandate dal provider di traduzione ad AEM, ma rimangono bloccate al livello `/content/projects` e non aggiornano i frammenti di contenuto. (NPR-41516)
...
* Si verifica un errore della console dopo un aggiornamento ad AEM 6.5, Service Pack 18. L&#39;errore si trova nel file `coralUI3.js` e si verifica quando si seleziona un elenco a discesa in AEM. Nello specifico, si verifica con un evento `onOverlayToggle`. Viene visualizzato l&#39;errore `Uncaught TypeError: Cannot read properties of null (reading 'innerText')`. (NPR-41467)
* In AEM, **[!UICONTROL Strumenti]** > **[!UICONTROL Generale]** > **[!UICONTROL Assegnazione tag]** > **[!UICONTROL Crea]** > **[!UICONTROL Crea tag]**, l&#39;immissione di caratteri non latini nel campo **Titolo** fa sì che il campo **Nome** venga compilato con il solo carattere trattino ( `-` ). (NPR-41623)
* L&#39;anno di copyright non è corretto nella finestra di dialogo `About Adobe Experience Manager`. (NPR-41526)
* Sono presenti **[!UICONTROL stringhe di proprietà profilo]** non tradotte durante la modifica delle impostazioni utente. Si verifica in tutte le lingue. (NPR-41365)
...
* [!DNL Experience Manager] 6.5.20.0 richiede [!DNL Experience Manager] 6.5. Per istruzioni dettagliate, consulta la [documentazione sull&#39;aggiornamento](/help/sites-deploying/upgrade.md). <!-- UPDATE FOR EACH NEW RELEASE -->
* Il download del service pack è disponibile in Adobe [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.20.0.zip).
* In una distribuzione con MongoDB e più istanze, installare [!DNL Experience Manager] 6.5.20.0 in una delle istanze di authoring utilizzando Gestione pacchetti.<!-- UPDATE FOR EACH NEW RELEASE -->

>[!IMPORTANT]
>
> Adobe consiglia di non rimuovere o disinstallare il pacchetto [!DNL Experience Manager] 6.5.20.0. Pertanto, prima di installare il pacchetto, è necessario creare un backup di `crx-repository` nel caso sia necessario eseguirne il rollback. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. Scarica il service pack da [Distribuzione software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.20.0.zip). <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!NOTE]
>
>Experience Manager 6.5.20.0 non supporta l&#39;installazione di Bootstrap. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. Nella pagina delle informazioni sul prodotto (`/system/console/productinfo`) viene visualizzata la stringa di versione aggiornata `Adobe Experience Manager (6.5.20.0)` in [!UICONTROL Prodotti installati]. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. La versione del bundle OSGi `org.apache.jackrabbit.oak-core` è 1.22.18 o successiva (utilizzare la console Web: `/system/console/bundles`). <!-- OAK Oak oak VERSION -MAY- NEED TO BE UPDATED FOR EACH NEW RELEASE -->
...
* [Elenco dei bundle OSGi inclusi in Experience Manager 6.5.20.0](/help/release-notes/assets/65200-bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Elenco dei pacchetti di contenuti inclusi in Experience Manager 6.5.20.0](/help/release-notes/assets/65200-packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
