...
Adobe Experience Manager 6.5.4.0 è un aggiornamento importante che include nuove funzionalità, miglioramenti richiesti dai clienti chiave e prestazioni, stabilità e miglioramenti della sicurezza, rilasciato dopo la disponibilità generale della versione 6.5 del **aprile 2019**. Può essere installato su Adobe Experience Manager 6.5.

Alcune funzionalità chiave e miglioramenti introdotti in Adobe Experience Manager 6.5.4.0 includono:
...
* È ora possibile sincronizzare le sottostrutture di contenuto selettivo con *Dynamic Media - modalità Scene7* invece di tutte le sottostrutture disponibili in `content/dam`.
...
* Le strutture di input o output di SOAP e le strutture di dati complesse ora supportano la sostituzione dinamica dei gruppi.
...
* Quando l&#39;URL di una pagina di Adobe Experience Manager Sites contiene due punti (`:`) o un simbolo di percentuale (`%`), il browser non risponde e i picchi di utilizzo di CPU (NPR-32369, NPR-31918).
...
* Quando selezioni l’opzione Trascina qui il componente nel segnaposto di una griglia reattiva, viene inviata una richiesta GET e la richiesta restituisce l’errore HTTP 403 (NPR-31845).
...
* Impossibile sincronizzare in Adobe la campagna che contiene il componente predefinito Importazione progettazione (NPR-31728).
...
* In SPS (Scene7 Publishing System) viene creata una cartella senza nome durante lo spostamento di una risorsa da una cartella a un’altra in Experience Manager con configurazione Dynamic Media Scene7 (NPR-32440).
...
* La pagina dei dettagli di Assets delle risorse PDF non mostra pulsanti di azione, ad eccezione dei pulsanti A raccolta e Aggiungi rappresentazione in Experience Manager in esecuzione in modalità di esecuzione Dynamic Media Scene7 (CQ-4286705).
...
* L’editor carosello non si apre in IE 11 sulla modalità ibrida di Experience Manager Dynamic Media (CQ-4255590). **Per i clienti in modalità ibrida di Adobe Dynamic Media:** dopo maggio 2022, Adobe cesserà il supporto per Internet Explorer 11 in modalità ibrida.
...
* I documenti MS Word e MS Excel, contenenti l’immagine JPEG, se analizzati con il parser Tika non riescono ad analizzare e viene rilevato un errore di classe non trovata (NPR-31952).
...
* Sicurezza dei documenti: impossibile aprire offline un file PDF protetto con l’opzione DisableGlobalOfflineSynchronizationData impostata su True (NPR-32078).
