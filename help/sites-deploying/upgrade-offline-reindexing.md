...
L’idea è quella di creare l’indice prima dell’aggiornamento, rispetto alle definizioni dell’indice della versione AEM di destinazione utilizzando [Oak-run](/help/sites-deploying/indexing-via-the-oak-run-jar.md) strumento. Il diagramma precedente mostra l’approccio alla reindicizzazione offline.
...
Per ulteriori dettagli sul processo di estrazione del testo, vedi [Documentazione Oak-run](https://jackrabbit.apache.org/oak/docs/query/pre-extract-text.html).
...
>[!NOTE]
>
>Per ulteriori dettagli sulle definizioni dell&#39;indice di dumping, consultare il [Documentazione Oak](https://jackrabbit.apache.org/oak/docs/query/oak-run-indexing.html#async-index-data).
...
Per ulteriori dettagli, consulta [creazione di punti di controllo](https://jackrabbit.apache.org/oak/docs/query/oak-run-indexing.html#out-of-band-create-checkpoint) dalla documentazione di Oak.
...
Nel caso di MongoMK, questo processo può essere accelerato se questo passaggio viene eseguito in un’istanza più vicina all’istanza MongoDB. Se viene eseguito sullo stesso computer, è possibile evitare il sovraccarico di rete.
...
Con AEM 6.4 e versioni più recenti, AEM ha la funzionalità incorporata di importare gli indici dal disco nella sequenza di avvio. La cartella `<repository>/indexing-result/indexes` viene controllato per verificare la presenza di dati di indice durante l&#39;avvio. È possibile copiare l’indice precreato nella posizione precedente durante la [processo di aggiornamento](in-place-upgrade.md#performing-the-upgrade) prima di iniziare con la nuova versione di **destinazione** JAR per AEM. AEM lo importa nell’archivio e rimuove il punto di controllo corrispondente dal sistema. Pertanto, una reindicizzazione è completamente evitata.
