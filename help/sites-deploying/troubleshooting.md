...
description: Questo articolo illustra alcuni dei problemi di installazione che potrebbero verificarsi in AEM.
seo-description: Questo articolo illustra alcuni dei problemi di installazione che potrebbero verificarsi in AEM.
...
Questa sezione contiene informazioni dettagliate sui registri disponibili per la risoluzione dei problemi e include anche informazioni su alcuni dei problemi che potrebbero verificarsi con AEM.
...
* Prova a utilizzare il menu di scelta rapida di AEM WCM (in genere fai clic con il pulsante destro del mouse) e seleziona &quot;Apri con....&quot;
* Verificate l&#39;elenco di Java o Sun Java e provate a eseguire AEM WCM con esso. Se avete installato più versioni Java, selezionate quella supportata.
...
Utilizzare le opzioni di comando Java per definire le impostazioni di memoria della JVM (ad esempio, java -Xmx512m -jar crx&amp;ast;.jar per impostare l&#39;altezza su 512 MB).

Specificate l&#39;opzione di impostazione della memoria quando avviate AEM WCM dalla riga di comando. Gli script di avvio/arresto di AEM WCM o gli script personalizzati per la gestione dell&#39;avvio di AEM WCM possono essere modificati anche per definire le impostazioni di memoria richieste.
...
Per creare automaticamente un heap dump quando la memoria è esaurita, utilizza il seguente comando:

java -Xmx256m -XX:+HeapDumpOnOutOfMemoryError -jar &amp;ast;.jar

Questo genera un file di dump heap (**java_...hprof**) ogni volta che il processo non dispone di memoria sufficiente. Il processo può continuare a essere eseguito dopo la generazione del dump dell&#39;heap. Solitamente, un file di dump heap è sufficiente per analizzare il problema.
...
In alcune situazioni, le schermate di benvenuto di AEM WCM non vengono visualizzate automaticamente anche se l&#39;archivio stesso è in esecuzione correttamente. Ciò può dipendere dalla configurazione del sistema operativo, dalla configurazione del browser o da fattori simili.

Il sintomo abituale è che la finestra AEM WCM Quickstart visualizza &quot;AEM WCM start up, in attesa dell’avvio del server....&quot; Se il messaggio viene visualizzato per un periodo di tempo relativamente lungo, immettete manualmente l’URL WCM AEM nella finestra del browser, utilizzando la porta predefinita 4502 o la porta su cui è in esecuzione l’istanza: http://localhost:4502/.

Inoltre, i registri potrebbero rivelare il motivo per cui il browser non si avvia.

A volte, la finestra AEM WCM Quickstart contiene il messaggio &quot;AEM WCM in esecuzione su http://localhost:port/&quot; e il browser non si avvia automaticamente. In questo caso, fate clic sull’URL nella finestra di avvio rapido di AEM WCM (si tratta di un collegamento ipertestuale) o immettete manualmente l’URL nel browser.
...
Consultate i passaggi dell’applicazione *Web* Implementare AEM per i dettagli.

### **La dimensione dell&#39;intestazione della risposta può essere maggiore di 4 Kb{#response-header-size-can-be-greater-than-kb}**

502 errori possono indicare che il server Web non è in grado di gestire le dimensioni dell’intestazione della risposta AEM HTTP. AEM può generare intestazioni di risposta HTTP che includono cookie di dimensioni maggiori di 4 Kb. Accertatevi che il contenitore del servlet sia configurato in modo che la dimensione massima dell’intestazione della risposta possa superare i 4 kb.
