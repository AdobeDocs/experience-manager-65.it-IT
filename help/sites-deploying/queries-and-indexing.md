...
L&#39;indice **** Traversal viene utilizzato se non è disponibile alcun altro indicizzatore. Ciò significa che il contenuto non è indicizzato e che i nodi di contenuto vengono attraversati per trovare corrispondenze alla query.
...
>[!NOTE]
>
>Per un repository di grandi dimensioni, la creazione di un indice richiede molto tempo. Ciò vale sia per la creazione iniziale di un indice, sia per la reindicizzazione (ricreazione di un indice dopo la modifica della definizione). Consultate anche [Risoluzione dei problemi degli indici](/help/sites-deploying/troubleshooting-oak-indexes.md) di rovere e [Prevenzione del reindicizzazione](/help/sites-deploying/troubleshooting-oak-indexes.md#preventing-slow-re-indexing)lenta.
...
Gli indici sono configurati come nodi nell&#39;archivio sotto il nodo **oak:index** .
...
* Il flag **reindex** che, se impostato su **true**, attiverà un reindicizzatore di contenuto completo.

### Indice ordinato {#the-ordered-index}

L&#39;indice ordinato è un&#39;estensione dell&#39;indice Property. Tuttavia, è stato dichiarato obsoleto. Gli indici di questo tipo devono essere sostituiti con l&#39;indice delle proprietà [Lucene](#the-lucene-property-index).
...
1. Aprite CRXDE e create un nuovo nodo sotto **oak:index**.
...
Per definire un indice delle proprietà Lucene per la query precedente, potete aggiungere la seguente definizione creando un nuovo nodo sotto **oak:index:**
...
>[!NOTE]
>
>Per informazioni più specifiche sull’indice delle proprietà Lucene, consultate la pagina [di documentazione di](https://jackrabbit.apache.org/oak/docs/query/lucene.html)Apache Jackrabbit Oak Lucene.

### Lucene Analytics {#lucene-analyzers}
...
Gli analizzatori possono essere configurati tramite il `analyzers` nodo (di tipo `nt:unstructured`) all&#39;interno della `oak:index` definizione.

L&#39;analizzatore predefinito per un indice è configurato nell&#39; `default` elemento secondario del nodo analizzatore.
...
Per casi come il caricamento di parole di arresto in cui è necessario caricare contenuto da file esterni, il contenuto può essere fornito creando un nodo figlio di `nt:file` tipo per il file in questione.
...
1. Quindi, modificate &quot;**Oak Solr embedded server configuration**&quot; e create una configurazione. Per maggiori informazioni sulle opzioni di configurazione, visitate il sito Web [Apache Solr](https://lucene.apache.org/solr/documentation.html).
...
1. Copiate le seguenti cartelle dall’istanza di esempio alle due cartelle condivise ( `aemsolr1\node1` e `aemsolr2\node2`):
...
1. Inserite i file di configurazione Solr e Zookeeper nelle `cfg` cartelle appena create.

   >[!NOTE]
   >
   >Per ulteriori informazioni sulla configurazione Solr e ZooKeeper, consulta la documentazione [relativa alla configurazione](https://wiki.apache.org/solr/ConfiguringSolr) Solr e la Guida [introduttiva](https://zookeeper.apache.org/doc/r3.1.2/zookeeperStarted.html)ZooKeeper.

1. Iniziate la prima condivisione con il supporto di ZooKeeper andando `aemsolr1\node1` ed eseguendo il comando seguente:

   ```xml
   java -Xmx2g -Dbootstrap_confdir=./cfg/oak/conf -Dcollection.configName=myconf -DzkRun -DnumShards=2 -jar start.jar
   ```

1. Avviate la seconda condivisione andando `aemsolr2\node2` ed eseguendo il comando seguente:
...
1. Scegliete **Remote** nell&#39;elenco a discesa in **Oak Solr** server provider.
...
È possibile accedervi dalla console Web cercando &quot;**Assicurare l’indice** delle proprietà Oak&quot;.
...
1. Fate clic sul pulsante **Aggiungi nuovo logger** nella parte inferiore della console.
1. Nella riga appena creata, aggiungete le categorie sopra indicate. Potete usare il segno **+** per aggiungere più categorie a un singolo logger.
1. Scegliete **DEBUG** dall’elenco a discesa a livello **di** registro.
...
1. Dopo aver eseguito la query, tornate alla console di registrazione e modificate il livello di registro del logger appena creato in **INFO**.
...
Poiché nella maggior parte dei casi, la configurazione di indicizzazione è memorizzata sotto il `/oak:index` nodo in CRXDE, è possibile ottenere la versione JSON in:
