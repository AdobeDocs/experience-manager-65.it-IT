...
seo-description: Scopri come utilizzare lo strumento di migrazione CRX2Oak.
...
L&#39;elenco delle modifiche e delle correzioni per la versione più recente si trova nelle [Note sulla versione CRX2Oak](https://docs.adobe.com/content/help/en/experience-manager-64/release-notes/crx2oak.html).
...
#### Supporto per le operazioni mappate su memoria {#support-for-memory-mapped-operations}
...
Per ulteriori informazioni, consulta la [Documentazione Apache](https://jackrabbit.apache.org/oak/docs/migration.html) .
...
* `--ldap`: Esegue la migrazione degli utenti LDAP da un&#39;istanza CQ 5.x a una basata su Oak. Affinché questo funzioni, il provider di identità nella configurazione Oak deve essere denominato ldap. Per ulteriori informazioni, consulta la [documentazione LDAP](/help/sites-administering/ldap-config.md).
...
### Opzioni del Negozio BLOB di destinazione {#destination-blobstore-options}
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>È possibile aggiungere le opzioni <strong>—TRACE a livello di registro</strong> o <strong>—DEBUG a livello di registro </strong>alla riga di comando durante l'esecuzione di CRX2Oak. In questa modalità i registri vengono automaticamente reindirizzati al file <strong>upgrade.log</strong>.</td>
  </tr>
  <tr>
   <td>Modalità autonoma</td>
   <td><p>Aggiungi le opzioni <strong>—trace</strong> alla riga di comando CRX2Oak per visualizzare gli eventi TRACE sull'output standard (è necessario reindirizzare i registri manualmente utilizzando il carattere di reindirizzamento: comando '&gt;' o 'tee' per un'ispezione successiva).</p> </td>
  </tr>
 </tbody>
</table>
...
>[!NOTE]
>
>Per ulteriori informazioni, consulta la documentazione di MongoDB Connection String su [Write Concerns](https://docs.mongodb.org/manual/reference/connection-string/#write-concern-options).
