...
# Implementazione di un valutatore predefinito personalizzato per il Generatore di query{#implementing-a-custom-predicate-evaluator-for-the-query-builder}

In questa sezione viene descritto come estendere [Query Builder](/help/sites-developing/querybuilder-api.md) implementando un valutatore predicato personalizzato.
...
>[!NOTE]
>
>Per ulteriori informazioni sul `PredicateEvaluator` pacchetto e sul `com.day.cq.search` pacchetto, consulta la documentazione [](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/index.html?com/day/cq/search/package-summary.html)Java.
...
Questa query è valida ma difficile da leggere e non evidenzia la relazione tra le tre proprietà di replica. L&#39;implementazione di un valutatore di predicati personalizzato ridurrà la complessità e migliorerà la semantica della query.

#### Obiettivi {#objectives}

L&#39;obiettivo di `ReplicationPredicateEvaluator` è supportare la query di cui sopra utilizzando la sintassi seguente.
...
>[!NOTE]
>
>L&#39;ambito della `cq-search` dipendenza è impostato su `provided` perché `cq-search` verrà fornito dal `OSGi` contenitore.
...
Il `cq-search` progetto contiene la classe `AbstractPredicateEvaluator` astratta. Questo può essere esteso con alcuni passaggi per implementare il vostro valutatore di predicati personalizzato `(PredicateEvaluator`).

>[!NOTE]
>
>Nella procedura seguente viene illustrato come creare un&#39; `Xpath` espressione per filtrare i dati. Un&#39;altra opzione consiste nell&#39;implementare il `includes` metodo che seleziona i dati su base di riga. See the [Java documentation](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/search/eval/PredicateEvaluator.html#includes28comdaycqsearchpredicatejavaxjcrqueryrowcomdaycqsearchevalevaluationcontext29) for more information.

1. Creare una nuova classe Java che si estende `com.day.cq.search.eval.AbstractPredicateEvaluator`
1. Annotate la classe con un `@Component` esempio
...
>[!NOTE]
>
>Deve `factory`essere una stringa univoca che inizia `com.day.cq.search.eval.PredicateEvaluator/`e termina con il nome dell&#39;utente personalizzato `PredicateEvaluator`.

>[!NOTE]
>
>Il nome del predicato `PredicateEvaluator` è utilizzato per la creazione di query.
...
1. Ignora:
   ...
   Nel metodo override si crea un&#39; `Xpath` espressione basata sull&#39;argomento `Predicate` specificato.
