...
description: Panoramica dei concetti di base sulla struttura di Adobe Experience Manager (AEM) e su come svilupparlo, inclusa la comprensione di JCR, Sling, OSGi, Dispatcher, flussi di lavoro e MSM.
...
>[!NOTE]
>
>Prima di passare ai concetti di base di Adobe Experience Manager (AEM), Adobe consiglia di completare l&#39;esercitazione WKND nel documento [Guida introduttiva allo sviluppo di AEM Sites](/help/sites-developing/getting-started.md). Include una panoramica del processo di sviluppo di AEM e un’introduzione ai concetti di base.

## Prerequisiti per lo sviluppo in AEM {#prerequisites-for-developing-on-aem}

Per sviluppare al di sopra di AEM sono necessarie le seguenti competenze:
...
Experience Server fornisce i servizi di esperienza su cui è basato AEM, che possono essere utilizzati per creare applicazioni personalizzate e incorpora il Content Repository basato su Jackrabbit.
...
AEM è stato creato utilizzando [Sling](https://sling.apache.org/index.html), un framework di applicazioni Web basato sui principi REST che consente di sviluppare facilmente applicazioni orientate ai contenuti. Sling utilizza come archivio dati un archivio JCR, come Apache Jackrabbit o, se esiste AEM, CRX Content Repository. Sling ha contribuito alla Apache Software Foundation; ulteriori informazioni sono disponibili su Apache.
...
**percorso contenuto** Percorso che specifica il contenuto di cui eseguire il rendering. Utilizzato con l’estensione. In questo esempio, traducono in `tools/spy.html`.
...
**params** Qualsiasi parametro necessario per il contenuto dinamico.
...
* se non viene trovato alcun nodo, l&#39;estensione viene eliminata e la ricerca ripetuta, ad esempio `../content/corporate/jobs/developer`
...
Quando si trova la risorsa appropriata (nodo di contenuto), viene estratto il tipo di risorsa **sling**. Questo è un percorso che individua lo script da utilizzare per il rendering del contenuto.
...
Tutti gli script Sling sono archiviati in sottocartelle di `/apps` o `/libs`, in cui viene eseguita la ricerca in questo ordine (vedi [Personalizzazione di componenti e altri elementi](/help/sites-developing/dev-guidelines-bestpractices.md#customizing-components-and-other-elements)).
...
Utilizzando l&#39;esempio precedente, se `sling:resourceType` è `hr/jobs` allora per:
...
Questa architettura consente di estendere Sling con moduli specifici per le applicazioni. Sling, e quindi CQ5, utilizza l’implementazione [Apache Felix](https://felix.apache.org/documentation/index.html) di OSGI (Open Services Gateway initiative) ed è basata sulle specifiche della piattaforma di servizio OSGi versione 4 versione 4.2. Sono entrambe raccolte di bundle OSGi in esecuzione all’interno di un framework OSGi.
...
Per ulteriori informazioni, vedere [Console Web](/help/sites-deploying/web-console.md), [Configurazione OSGI](/help/sites-deploying/configuring-osgi.md) e [Impostazioni configurazione OSGi](/help/sites-deploying/osgi-configuration-settings.md).
...
Definisce il componente Pagina utilizzato per eseguire il rendering della pagina e il contenuto predefinito (contenuto principale di primo livello). Il contenuto definisce il modo in cui viene eseguito il rendering quando AEM è incentrato sui contenuti.
...
* `/apps`
  ...
  Relativo all’applicazione; include definizioni di componenti specifiche del sito web. I componenti sviluppati possono essere basati sui componenti predefiniti disponibili in `/libs/foundation/components`.
...
* `/libs`
  ...
  Librerie e definizioni che appartengono al nucleo di AEM. Le sottocartelle in `/libs` rappresentano le funzionalità predefinite di AEM, ad esempio la ricerca o la replica. Il contenuto in `/libs` non deve essere modificato in quanto influisce sul funzionamento di AEM. Le funzionalità specifiche del sito Web devono essere sviluppate in `/apps` (vedi [Personalizzazione di componenti e altri elementi](/help/sites-developing/dev-guidelines-bestpractices.md#customizing-components-and-other-elements)).
...
Con AEM, un ambiente di produzione è spesso costituito da due diversi tipi di istanze: un [istanza Autore e un&#39;istanza Publish](/help/sites-deploying/deploy.md#author-and-publish-installs).

## Dispatcher {#the-dispatcher}

Dispatcher è lo strumento di Adobe per il caching e/o il bilanciamento del carico. Ulteriori informazioni sono disponibili in [Dispatcher](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/dispatcher.html?lang=it).
...
Per informazioni dettagliate, consulta la documentazione dello strumento [FileVault](/help/sites-developing/ht-vlttool.md).

## Flussi di lavoro {#workflows}

I contenuti sono spesso soggetti a processi organizzativi, tra cui passaggi quali l’approvazione e l’approvazione da parte di vari partecipanti. Questi processi possono essere rappresentati come flussi di lavoro, [definiti e sviluppati all&#39;interno di AEM](/help/sites-developing/workflows-models.md), quindi applicati alle [pagine di contenuto appropriate](/help/sites-administering/workflows.md) o alle [risorse digitali](/help/assets/assets-workflow.md) come richiesto.
