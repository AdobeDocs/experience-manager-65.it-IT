...
1. Regolare la limitazione del tipo di nodo della query in modo che la query corrisponda a un indice esistente della proprietà Lucene.
   ...
   * Nessun indice fornito da AEM esiste `nt:hierarchyNode`, tuttavia, in quanto è presente un indice fornito per `cq:Page`.
   L’impostazione `type=cq:Page` limita questa query ai soli `cq:Page` nodi e risolve la query a cqPageLucene di AEM, limitando i risultati a un sottoinsieme di nodi (solo nodi cq:Page) in AEM.
...
1. ***Questa regolazione è specifica di Query Builder e non si applica a JCR-SQL2 o XPath.***
   ...
   Utilizzate [Query Builder&#39;Total](/help/sites-developing/querybuilder-api.md#using-p-guesstotal-to-return-the-results) quando l&#39;intero set di risultati è **not **immediatamente necessario.
...
* **CRXDE Lite - Strumento Query**
   ...
   * In AEM, all’indirizzo [/crx/de/index.jsp](http://localhost:4502/crx/de/index.jsp) > Strumenti > Query...
...
* **NodeCounter JMX**
   ...
   * In AEM all&#39;indirizzo [/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3DnodeCounter%2Ctype%3DNodeCounter](http://localhost:4502/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3DnodeCounter%2Ctype%3DNodeCounter)
