...
È inoltre necessario configurare la visualizzazione della barra degli strumenti ContextHub in modalità Anteprima. Consultate [Mostrare e nascondere l’interfaccia utente](ch-configuring.md#showing-and-hiding-the-contexthub-ui)ContextHub.
...
Tutti i tipi di store sono estensioni della [`ContextHub.Store.Core`](contexthub-api.md#contexthub-store-core) classe. Per informazioni sulla creazione di un nuovo tipo di store, consultate [Creazione di store](ch-extend.md#creating-custom-store-candidates)personalizzati. Per informazioni sui tipi di store di esempio, consultate [Esempio di candidati](ch-samplestores.md)per lo store ContextHub.
...
I seguenti segmenti sono installati con il sito di esercitazione [WKND.](getting-started.md)
...
* Per determinare la latitudine dell&#39;utente viene utilizzato innanzitutto lo [store di geolocalizzazione](ch-samplestores.md#contexthub-geolocation-sample-store-candidate) .
* Quindi l&#39;elemento dei dati del mese dello store [](ch-samplestores.md#contexthub-surferinfo-sample-store-candidate) surferinfo determina la stagione in cui si trova in quella latitudine.
...
Sono disponibili diverse opzioni per il debug di ContextHub, compresa la generazione dei registri. Per ulteriori informazioni, consulta [Configurazione di ContextHub.](ch-configuring.md#logging-debug-messages-for-contexthub)

## Vedere una panoramica del framework ContextHub {#see-an-overview-of-the-contexthub-framework}

ContextHub fornisce una pagina [di](ch-diagnostics.md) diagnostica in cui potete vedere una panoramica del framework ContextHub.
