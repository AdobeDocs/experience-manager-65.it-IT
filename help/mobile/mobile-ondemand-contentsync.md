...
description: Seguite questa pagina per informazioni sulla sincronizzazione dei contenuti. Le pagine create in AEM possono essere utilizzate come contenuto dell'app, anche quando il dispositivo è offline. Inoltre, poiché le pagine AEM sono basate su standard Web, funzionano su più piattaforme e possono essere incorporate in qualsiasi wrapper nativo. Questa strategia riduce il lavoro di sviluppo e consente di aggiornare facilmente il contenuto delle app.
seo-description: Seguite questa pagina per informazioni sulla sincronizzazione dei contenuti. Le pagine create in AEM possono essere utilizzate come contenuto dell'app, anche quando il dispositivo è offline. Inoltre, poiché le pagine AEM sono basate su standard Web, funzionano su più piattaforme e possono essere incorporate in qualsiasi wrapper nativo. Questa strategia riduce il lavoro di sviluppo e consente di aggiornare facilmente il contenuto delle app.
...
## Sviluppo di gestori di sincronizzazione dei contenuti {#developing-the-content-sync-handlers}
...
* Il gestore deve segnalare true solo se aggiorna la cache ContentSync. Se si segnala erroneamente come &quot;true&quot;, AEM creerà un aggiornamento quando non si è verificato un aggiornamento.
...
>[!NOTE]
>
>Abilita la registrazione *di debug* ContentSync tramite le configurazioni di registrazione OSGI nel pacchetto *com.day.cq.contentsync*. Questo consente di tenere traccia dei gestori eseguiti e se hanno aggiornato la cache e segnalato l&#39;aggiornamento della cache.
...
Per ulteriori informazioni, consulta *Esempio di configurazione* della sincronizzazione dei contenuti.
...
Se il `cq:ContentSyncConfig` nodo non dispone di `updateuser` proprietà, l&#39; `anonymous` utente predefinito aggiorna la cache.
...
**contenuto** Consente di eseguire il rendering del contenuto utilizzando l&#39;elaborazione [standard della richiesta](/help/sites-developing/the-basics.md#sling-request-processing)Sling.
...
* [Creazione di contenuti AEM per i servizi on-demand AEM Mobile](/help/mobile/mobile-apps-ondemand.md)
