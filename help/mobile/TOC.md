---
cloud: Experience Cloud
product: adobe experience manager
audience: end-user
user-guide-title: Guida utente di AEM 6.5 Mobile
breadcrumb-title: Guida di Mobile
user-guide-description: Crea e gestisci contenuti per le app mobili.
feature: Mobile
role: User
source-git-commit: 603518dbe3d842a08900ac40651919c55392b573
workflow-type: tm+mt
source-wordcount: '331'
ht-degree: 10%

---


# Guida utente di AEM 6.5 Mobile {#mobile}

+ [Panoramica della guida utente di Mobile](home.md)
+ Introduzione ad AEM Mobile {#introduction}
   + [Guida introduttiva ad AEM Mobile](getting-started-aem-mobile.md)
   + [AEM Mobile On-Demand](getting-started-aem-mobile-on-demand.md)
   + [App mobili](mobile-apps.md)
   + [AEM Adobe PhoneGap](getting-started-aem-mobile-phonegap.md)
   + [AEM Mobile - Preparazione al RGPD](aem-mobile-gdpr-compliance.md)
   + [Sviluppa app mobili](develop-mobile-apps.md)
+ App Authoring On-Demand Services {#authoring}
   + [Mobile On-Demand](mobile-apps-ondemand.md)
   + [Gestione del contenuto](mobile-apps-ondemand-manage-content-ondemand.md)
   + [Pubblica contenuto di annullamento pubblicazione](mobile-on-demand-publishing-unpublishing.md)
   + [Dashboard dell’applicazione AEM Mobile](mobile-apps-ondemand-application-dashboard.md)
   + [Azioni di creazione e configurazione delle applicazioni](mobile-apps-ondemand-application-create-configure-action.md)
   + [Gestione degli articoli](mobile-on-demand-managing-articles.md)
   + [Caricamento delle risorse condivise](mobile-on-demand-shared-resources.md)
   + [Configurazione cloud](mobile-on-demand-associating-an-on-demand-app-to-cloud-configuration.md)
   + [Gestione dei banner](mobile-on-demand-managing-banners.md)
+ Sviluppo dell’app On-Demand Services {#developing}
   + [AEM Mobile On-Demand](aem-mobile-on-demand.md)
   + [Modelli di app e componenti](app-templates-and-components1.md)
   + [Proprietà e nodi di contenuto](content-properties.md)
   + [Creazione configurazione esportazione risorse condivise](creating-shared-resources-export-configuration.md)
   + [Creazione e aggiunta di modelli e componenti](mobile-ondemand-app-templates.md)
   + [Best practice](best-practices-aem-mobile.md)
   + [Creazione della configurazione di esportazione dell&#39;articolo](creating-article-export-configuration.md)
   + [Dispositivi mobili con sincronizzazione contenuti](mobile-ondemand-contentsync.md)
   + [Modelli di pagina per le app mobili](apps-architecture.md)
   + [Utilizzo delle proprietà del contenuto per esportare il contenuto](on-demand-content-properties-exporting.md)
   + [Rendering e consegna](rendering-and-delivery.md)
   + [Content Services](developing-content-services.md)
   + [Consegna dei contenuti](develop-content-as-a-service.md)
+ Amministrazione dell’app dei servizi on-demand {#administering}
   + [AEM Mobile On-Demand](aem-mobile.md)
   + [Configurazione AEM Mobile](aem-mobile-setup.md)
   + [Configurare utenti e gruppi di utenti](aem-mobile-configure-users.md)
   + [Anteprima con verifica preliminare](aem-mobile-manage-ondemand-services.md)
   + [App mobili](administer-mobile-apps.md)
   + [Panoramica dei modelli](model-management.md)
   + [Spazi ed entità](spaces-and-entities.md)
   + [Gestione delle raccolte](mobile-on-demand-managing-collections.md)
+ Authoring di Adobe PhoneGap Enterprise {#author-phonegap}
   + [PhoneGap](phonegap.md)
   + [Modifica dei metadati dell’app](phonegap-editmetadata.md)
   + [Definizioni delle app](phonegap-app-definitions.md)
   + [Creazione e gestione del contenuto delle app](phonegap-manage-app-content.md)
   + [Creazione di una nuova app AEM Mobile tramite la procedura guidata](phonegap-create-new-app.md)
   + [Authoring di applicazioni mobili](phonegap-authoring-apps.md)
   + [Gestisci sezione app](phonegap-app-details-tile.md)
   + [Personalizzazione dei contenuti AEM Mobile](phonegap-aem-mobile-content-personalization.md)
+ Sviluppo di Adobe PhoneGap Enterprise {#developphonegap}
   + [PhoneGap](developing-in-phonegap.md)
   + [Creazione di applicazioni mobili](building-app-mobile-phonegap.md)
   + [Notifiche push](phonegap-push-notifications.md)
   + [Sviluppo di applicazioni mobili in AEM](developing-mobile-applications.md)
   + [Dispositivi mobili con sincronizzazione contenuti](phonegap-contentsync.md)
   + [Anatomia di un’app](phonegap-apps-arch.md)
   + [Sviluppo di app con PhoneGap CLI](phonegap-apps-pg-cli.md)
   + [Gestori di app pronti all’uso](contentsync-app-handlers.md)
   + [Creazione e modifica di app tramite la console App](phonegap-apps-console.md)
   + [La tua app ibrida è pronta per AEM Mobile?](phonegap-adding-content-to-imported-app.md)
   + [Accedere alle funzioni del dispositivo](phonegap-access-device-features.md)
   + [Aggiungere Adobe Analytics alla tua applicazione mobile](phonegap-add-analytics-to-apps.md)
   + [Modelli di pagina per le app mobili](phonegap-apps-arch-page-templates.md)
   + [Configurazione del Cloud Service Adobe Target](aem-mobile-configuring-cloud-service.md)
   + [Applicazioni a pagina singola](phonegap-single-page-applications.md)
   + [Modelli nell’archivio](https://experienceleague.adobe.com/docs/experience-manager-65/mobile/administering/administer-mobile-apps.html)
   + [Avvio di PhoneGap AEM](starting-aem-phonegap-app.md)
   + [Verifica delle app mobili](develop-mobile-apps-testing.md)
   + [Strutturare un’app](phonegap-structure-an-app.md)
   + [Tracciare le prestazioni dell’app con Adobe Mobile Analytics](phonegap-intro-to-app-analytics.md)
+ Amministrazione di Adobe PhoneGap Enterprise {#administering-phonegap}
   + [Quickstart per dispositivi mobili](phonegap-mobile-quickstart.md)
   + [Amministrazione di contenuti per Adobe PhoneGap Enterprise con AEM](administer-phonegap.md)
   + [Configurare il Cloud Service Adobe PhoneGap Build](configure-phonegap-build-cloud.md)
   + [Impostazione del filtro Referrer su Allow Empty](setting-referrer-filter-empty.md)
   + [Configurare il Cloud Service Adobe Mobile Services](configure-adobe-mobile-cloud-service.md)
   + [Configurare utenti e gruppi di utenti](configure-users-groups.md)