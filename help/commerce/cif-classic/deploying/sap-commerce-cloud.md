...
description: Scopri come implementare l’eCommerce con SAP Commerce Cloud.
...
Distribuzione di [pacchetti eCommerce necessari](#packages-needed-for-ecommerce-with-hybris) fornirà la piena funzionalità del quadro di eCommerce, insieme a un’implementazione di riferimento della funzionalità eCommerce fornita con un’implementazione ibrida (compreso un catalogo dimostrativo)
...
>* È necessario Java 7 per eseguire [server hybris 5.](https://www.hybris.com/en/architecture-technology)
>* Il componente aggiuntivo hybris, il [Acceleratore Telco](https://www.hybris.com/en/products/telecommunication), non è supportato dall’estensione AEM.
>
...
1. Crea il catalogo nel motore di eCommerce:
   ...
   1. [Imposta il Negozio Geometrixx Outdoor](#setup-the-geometrixx-outdoors-store).
...
I passaggi descritti in questa procedura consentono di scaricare e generare il server ibrido. Verranno inoltre eseguite le configurazioni iniziali necessarie per le connessioni tra hybris e cq. L’estensione sarà quindi utilizzabile con le impostazioni predefinite.
...
1. Scarica il file **hybris Commerce Suite** distribuzione dal sito di download hybris.
   ...
   >[!CAUTION]
   >
   >Per accedervi dovrai disporre di un account (da hybris).
...
1. Clic **Inizializza** e quindi confermare l’azione di inizializzazione (in quanto eliminerà i dati esistenti).

   L’avanzamento verrà visualizzato nella console, con `FINISHED` che indica il completamento.
