...
Potete creare un&#39;applicazione client utilizzando Flash Builder che esegue l&#39;autenticazione Single Sign-On (SSO) tramite token HTTP. Supponete, ad esempio, di creare un&#39;applicazione basata sul Web utilizzando Flash Builder. Si supponga quindi che l&#39;applicazione contenga viste diverse, in cui ogni vista richiama un&#39;operazione AEM Forms diversa. Anziché autenticare un utente per ogni operazione Forms, è possibile creare una pagina di login che consenta a un utente di effettuare l&#39;autenticazione una volta sola. Una volta autenticato, un utente può richiamare più operazioni senza dover ripetere l&#39;autenticazione. Ad esempio, se un utente ha eseguito l&#39;accesso a Workspace (o a un&#39;altra applicazione Forms), non dovrà effettuare nuovamente l&#39;autenticazione.

Sebbene l&#39;applicazione client contenga la logica dell&#39;applicazione necessaria per eseguire l&#39;autenticazione SSO, AEM Forms User Management esegue l&#39;autenticazione utente effettiva. Per autenticare un utente utilizzando token HTTP, l&#39;applicazione client richiama l&#39; `authenticateWithHTTPToken` operazione del servizio Authentication Manager. Gestione utente è in grado di autenticare gli utenti mediante un token HTTP. Per le chiamate successive di servizi Web o di remota comunicazione agli AEM Forms, non è necessario trasmettere le credenziali per l&#39;autenticazione.

>[!NOTE]
>
>Prima di leggere questa sezione, si consiglia di avere familiarità con i AEM Forms Invoking utilizzando Remoting. (Vedere [Chiamata di AEM Forms mediante AEM Forms Remoting](/help/forms/developing/invoking-aem-forms-using-remoting.md#invoking-aem-forms-using-remoting).)

Il seguente processo di breve durata AEM Forms, denominato `MyApplication/EncryptDocument`, viene richiamato dopo che un utente è autenticato tramite SSO. Per informazioni su questo processo, ad esempio i valori di input e output, vedere l’esempio [di processo](/help/forms/developing/aem-forms-processes.md)breve durata.

![cf_cf_encryptdocumentprocess2](assets/cf_cf_encryptdocumentprocess2.png)

>[!NOTE]
>
>Questo processo non è basato su un processo AEM Forms esistente. Per seguire gli esempi di codice che illustrano come richiamare questo processo, create un processo denominato `MyApplication/EncryptDocument` utilizzando workbench. (Vedere [Uso di Workbench](https://www.adobe.com/go/learn_aemforms_workbench_63).)

L’applicazione client creata con Flash Builder interagisce con il servlet di sicurezza di User Manager configurato in `/um/login` e `/um/logout`. In altre parole, l&#39;applicazione client invia una richiesta all&#39; `/um/login` URL durante l&#39;avvio per determinare lo stato dell&#39;utente. Quindi User Manager risponde con lo stato dell&#39;utente. L’applicazione client e il servlet di sicurezza User Manager comunicano mediante HTTP.

**Formato richiesta**

Il servlet di protezione richiede le seguenti variabili di input:

* `um_no_redirect` - Questo valore deve essere `true`. Questa variabile accompagna tutte le richieste effettuate al servlet di sicurezza User Manager. Inoltre, aiuta il servlet di sicurezza a distinguere la richiesta in entrata proveniente da un client Flex o da altre applicazioni Web.
* `j_username` - Questo valore è il valore dell&#39;identificatore di login dell&#39;utente fornito nel modulo di login.
* `j_password` - Questo valore corrisponde alla password dell&#39;utente fornita nel modulo di login.

Il `j_password` valore è richiesto solo per le richieste di credenziali. Se il valore della password non è specificato, il servlet di sicurezza verifica se l&#39;account utilizzato è già autenticato. In tal caso, è possibile procedere; tuttavia, il servlet di sicurezza non esegue nuovamente l&#39;autenticazione.

>[!NOTE]
>
>Per una corretta gestione di i18n, accertatevi che questi valori siano in formato POST.

**Formato di risposta**

Il servlet di sicurezza configurato in `/um/login` risposta utilizza il `URLVariables` formato. In questo formato, l&#39;output del tipo di contenuto è text/plain. L&#39;output contiene coppie di valori di nome separate da un carattere e commerciale (&amp;). La risposta contiene le seguenti variabili:

* `authenticated` - Il valore è `true` o `false`.
* `authstate` - Questo valore può contenere uno dei seguenti valori:

   * `CREDENTIAL_CHALLENGE` - Questo stato indica che User Manager non è in grado di determinare l&#39;identità dell&#39;utente con alcun mezzo. Per consentire l&#39;autenticazione, è necessario immettere il nome utente e la password dell&#39;utente.
   ...
   * `FAILED` - Questo stato indica che User Manager non è stato in grado di autenticare l&#39;utente. Come risposta a questo stato, il client Flex può mostrare all&#39;utente un messaggio di errore.
   * `LOGGED_OUT` - Questo stato indica che l&#39;utente ha eseguito correttamente l&#39;accesso.

* `assertionid` - Se lo stato è `COMPLETE` allora contiene il `assertionId` valore dell&#39;utente. Un&#39;applicazione client può ottenere l&#39; `AuthResult` utente.

**Processo di login**

All&#39;avvio di un&#39;applicazione client, potete effettuare una richiesta POST al servlet di `/um/login` sicurezza. Esempio, `https://<your_serverhost>:<your_port>/um/login?um_no_redirect=true`. Quando la richiesta raggiunge il servlet di sicurezza di User Manager, esegue i seguenti passaggi:

1. Cerca un cookie denominato `lcAuthToken`. Se l&#39;utente ha già eseguito l&#39;accesso a un&#39;altra applicazione Forms, il cookie è presente. Se il cookie viene trovato, il relativo contenuto viene convalidato.
...
1. Se SPNEGO è abilitato, il servlet tenta di avviare SPNEGO e tenta di determinare l&#39;identità dell&#39;utente.

Se il servlet di sicurezza individua un token valido che corrisponde a un utente, il servlet di sicurezza consente di proseguire e rispondere con `authstate=COMPLETE`. In caso contrario, il servlet di sicurezza risponde con `authstate=CREDENTIAL_CHALLENGE`. Nell&#39;elenco seguente sono illustrati i seguenti valori:

* `Case authstate=COMPLETE`: Indica che l&#39;utente è autenticato e il `assertionid` valore contiene l&#39;identificatore di asserzione per l&#39;utente. In questa fase, l&#39;applicazione client può connettersi ai AEM Forms. Il servlet configurato per quell&#39;URL può ottenere l&#39; `AuthResult` utente richiamando il `AuthenticationManager.authenticate(HttpRequestToken)` metodo. L’ `AuthResult` istanza può creare il contesto del manager utente e archiviarlo nella sessione.
...
Quando un&#39;applicazione client si disconnette, potete inviare una richiesta al seguente URL:

`https://<your_serverhost>:<your_port>/um/logout?um_no_redirect=true`

Una volta ricevuta la richiesta, il servlet di sicurezza User Manager elimina il `lcAuthToken` cookie e risponde con `authstate=LOGGED_OUT`. Dopo che l&#39;applicazione client ha ricevuto questo valore, l&#39;applicazione può eseguire attività di pulizia.

## Creazione di un&#39;applicazione client che esegue l&#39;autenticazione degli utenti dei moduli AEM tramite SSO {#creating-a-client-application-that-authenticates-aem-forms-users-using-sso}

Per dimostrare come creare un&#39;applicazione client che esegue l&#39;autenticazione SSO, viene creata un&#39;applicazione client di esempio. L&#39;illustrazione seguente mostra i passaggi eseguiti dall&#39;applicazione client per autenticare un utente tramite SSO.
...
1. L&#39;applicazione client attiva l&#39; `applicationComplete` evento.
1. La chiamata a `ISSOManager.singleSignOn` è fatta. L&#39;applicazione client invia una richiesta al servlet di sicurezza User Manager.
1. Se il servlet di sicurezza autentica l&#39;utente, `ISSOManager` invia `SSOEvent.AUTHENTICATION_SUCCESS`. Come risposta, l&#39;applicazione client mostra la pagina principale. In questo esempio, la pagina principale richiama il processo di breve durata dei AEM Forms denominato MyApplication/EncryptDocument.
1. Se il servlet di sicurezza non è in grado di determinare se l&#39;utente è valido, l&#39;applicazione richiede nuovamente le credenziali utente. La `ISSOManager` classe invia l&#39; `SSOEvent.AUTHENTICATION_REQUIRED` evento. L&#39;applicazione client visualizza la pagina di login.
1. Le credenziali fornite nella pagina di accesso vengono inviate al `ISSOManager.login` metodo. Se l&#39;autenticazione ha esito positivo, allora porta al punto 3. In caso contrario, l&#39; `SSOEvent.AUTHENTICATION_FAILED` evento viene attivato. L&#39;applicazione client visualizza la pagina di login e un messaggio di errore appropriato.

### Creazione dell&#39;applicazione client {#creating-the-client-application}

L&#39;applicazione client è costituita dai file seguenti:

* `SSOStandalone.mxml`: Il file MXML principale che rappresenta l&#39;applicazione client. Consultate [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-ssostandalone-mxml-file)SSOStandalone.mxml.
* `um/ISSOManager.as`: Esporre le operazioni relative a Single Sign On (SSO). Consultate [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-issomanager-as-file)ISSOManager.as.
* `um/SSOEvent.as`: L&#39;evento `SSOEvent` viene inviato per gli eventi correlati a SSO. Consultate [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-ssoevent-as-file)SSOEvent.as.
* `um/SSOManager.as`: Gestisce le operazioni relative all&#39;SSO e invia gli eventi appropriati. Consultate [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-ssomanager-as-file)SSOManager.as.
* `um/UserManager.as`: Contiene la logica dell&#39;applicazione che richiama il servizio Authentication Manager utilizzando il relativo WSDL. (Vedere [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-usermanager-as-file)UserManager.as.)
* `views/login.mxml`: Rappresenta la schermata di login. Consultate [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-login-mxml-file)login.mxml.
* `views/logout.mxml`: Rappresenta la schermata di logout. Consultate [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-logout-mxml-file)logout.mxml.
* `views/progress.mxml`: Rappresenta una visualizzazione di avanzamento. Consultate [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-progress-mxml-file)progress.mxml.
* `views/remoting.mxml`: Rappresenta la visualizzazione che richiama il processo di AEM Forms di breve durata denominato MyApplication/EncryptDocument tramite la rimozione. Consultate [Creazione del file](creating-flash-builder-applications-perform.md#creating-the-remoting-mxml-file)remoting.mxml.
...
>[!NOTE]
>
>Tenere presente che esistono due pacchetti denominati um e viste. Quando create l&#39;applicazione client, accertatevi di inserire i file nei pacchetti appropriati. Inoltre, accertatevi di aggiungere il file adobe-remoting-provider.swc al percorso di classe del progetto. Consultate [Inclusione del file](/help/forms/developing/invoking-aem-forms-using-remoting.md#including-the-aem-forms-flex-library-file)libreria Flex AEM Forms.
...
Il codice seguente rappresenta il file remoting.mxml che richiama il `MyApplication/EncryptDocument` processo. Poiché un documento viene passato al processo, in questo file si trova la logica applicativa responsabile per il passaggio di un documento protetto ai AEM Forms. (vedere [Trasmissione di documenti protetti per richiamare i processi tramite Remoting](/help/forms/developing/invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).)
...
In questa situazione, l&#39;utente tenta di effettuare per la prima volta l&#39;accesso da un&#39;applicazione client ai AEM Forms. (non esiste una sessione precedente che coinvolga l’utente). In `applicationComplete` questo caso, viene richiamato il `SSOManager.singleSignOn` metodo che invia una richiesta al manager utente.
...
Come risposta a questo valore, viene inviato un `SSOEvent.AUTHENTICATION_REQUIRED` valore. Di conseguenza, l&#39;applicazione client visualizza una schermata di login per l&#39;utente. Le credenziali vengono inviate nuovamente al servlet di sicurezza di User Manager.
...
In questa situazione, l&#39;utente ha già eseguito l&#39;accesso ai AEM Forms e quindi passa all&#39;applicazione client. L&#39;applicazione client si connette al servlet di sicurezza User Manager durante l&#39;avvio.
...
Poiché l’utente è già autenticato, il cookie di User Manager è presente e viene inviato al servlet di sicurezza di User Manager. Il servlet quindi ottiene il `assertionId` valore e verifica se è valido. Se è valido, `authstate=COMPLETE` viene restituito. In caso contrario `authstate=CREDENTIAL_CHALLENGE` viene restituito. Di seguito è riportata una tipica risposta:
...
In questa situazione, all’utente non viene visualizzata una schermata di login, bensì viene portato direttamente a una schermata di benvenuto.
