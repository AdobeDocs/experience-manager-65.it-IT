...
description: Dettagli dell'architettura per AEM Forms e topologie consigliate per clienti e clienti AEM nuovi ed esistenti che eseguono l'aggiornamento da LiveCycle ES4 ai AEM Forms.
seo-description: Dettagli dell'architettura per AEM Forms e topologie consigliate per clienti e clienti AEM nuovi ed esistenti che eseguono l'aggiornamento da LiveCycle ES4 ai AEM Forms.
...
L’architettura dei AEM Forms include i seguenti componenti:
...
**AEM Forms su OSGi:** Un AEM Forms nell’ambiente OSGi è un AEM Author standard o un AEM Publish con un pacchetto AEM Forms distribuito al suo interno. È possibile eseguire AEM Forms su OSGi in un [singolo ambiente server, in una farm e in un cluster](/help/sites-deploying/recommended-deploys.md). La configurazione del cluster è disponibile solo per le istanze di AEM Author.

**AEM Forms su JEE:** I AEM Forms su JEE sono AEM Forms server in esecuzione sullo stack JEE. È dotato di AEM Author con pacchetti AEM Forms add-on e altre funzionalità JEE per i AEM Forms, che sono state implementate insieme a un singolo stack JEE in esecuzione su un server applicazioni. È possibile eseguire AEM Forms su JEE in configurazioni a server singolo e cluster. I AEM Forms su JEE sono necessari solo per eseguire la protezione dei documenti, la gestione dei processi e per gli utenti di LiveCycle che eseguono l&#39;aggiornamento ai AEM Forms. Di seguito sono riportati alcuni scenari aggiuntivi per utilizzare AEM Forms su JEE:

* **Supporto per l’area di lavoro HTML (per i clienti che utilizzano l’area di lavoro HTML):** I AEM Forms su JEE consentono il single sign-on con le istanze di elaborazione, distribuiscono determinate risorse sottoposte a rendering sulle istanze di elaborazione e gestiscono l&#39;invio di moduli sottoposti a rendering all&#39;interno dell&#39;area di lavoro HTML.
* **Elaborazione** avanzata dei dati di comunicazione interattiva/modulo: I AEM Forms su JEE possono essere utilizzati anche per l&#39;elaborazione di dati di comunicazione interattiva/modulo (e per il salvataggio dei risultati in un archivio di dati adatto) in casi d&#39;uso complessi in cui sono richieste funzionalità avanzate di gestione dei processi.

I AEM Forms su JEE offrono anche i seguenti servizi di supporto ai componenti AEM:

* **Gestione utente integrata:** Consente agli utenti di AEM Forms su JEE di essere riconosciuti come moduli AEM negli utenti OSGi e consente di abilitare SSO sia per gli utenti OSGi che per gli utenti JEE. Questo è richiesto per gli scenari in cui è richiesto il single sign-on tra moduli AEM su OSGi e AEM Forms su JEE (ad esempio, area di lavoro HTML).
* **Hosting risorse:** I AEM Forms su JEE possono distribuire risorse (ad esempio, moduli HTML5) su AEM Forms su OSGi.

L&#39;interfaccia utente per la creazione di AEM Forms non supporta la creazione di DOR (Document of Record), PDF forms e moduli HTML5. Tali risorse sono progettate utilizzando l&#39;applicazione autonoma di Designer Forms e caricate singolarmente in Gestione AEM Forms. In alternativa, per i AEM Forms su JEE, i moduli possono essere progettati come risorse dell&#39;applicazione (in Workbench AEM Forms) e distribuiti nei AEM Forms del server JEE.

I AEM Forms su OSGi e i AEM Forms su JEE sono entrambi dotati di funzionalità di flusso di lavoro. È possibile creare e implementare rapidamente flussi di lavoro di base per varie attività nei moduli AEM su OSGi, senza dover installare la funzionalità di gestione dei processi completa dei AEM Forms su JEE. Esistono differenze nelle [funzioni del flusso di lavoro incentrato sui moduli sui AEM Forms in OSGi e nelle funzionalità di gestione dei processi dei AEM Forms in JEE](capabilities-osgi-jee-workflows.md). Lo sviluppo e la gestione di flussi di lavoro incentrati sui moduli sui AEM Forms in OSGi utilizzano le familiari funzionalità Flusso di lavoro AEM e Casella in entrata AEM.

## Terminologie {#terminologies}

L’immagine seguente mostra diverse configurazioni del server AEM Form e i relativi componenti utilizzati in una tipica implementazione AEM Forms:

![aem_forms_-_consigliendedtopologia](assets/aem_forms_-_recommendedtopology.png)

**Autore:** Un’istanza di authoring è un server di AEM Forms in esecuzione nella modalità standard di esecuzione di Author. Può trattarsi di AEM Forms su JEE o AEM Forms sull’ambiente OSGi. È destinata agli utenti interni, ai progettisti e agli sviluppatori di comunicazioni interattive. Attiva le seguenti funzionalità:
...
I clienti che intendono utilizzare le funzionalità di acquisizione dei dati dei AEM Forms, ad esempio moduli adattivi, moduli HTML5, PDF forms, possono avere una topologia simile a quella visualizzata di seguito. Questa topologia è anche consigliata per l&#39;utilizzo delle funzionalità di comunicazione interattiva dei AEM Forms.
...
* L&#39;utilizzo dell&#39;app HTML Workspace e AEM Forms richiede un&#39;istanza di creazione o elaborazione di AEM. Puoi utilizzare l’istanza di creazione AEM integrata per i AEM Forms sul server JEE invece di configurare un ulteriore server di creazione AEM esterno.
* Un&#39;istanza AEM Author o Elaborazione è necessaria solo per i flussi di lavoro incentrati su Forms su OSGi, moduli adattivi, portale moduli e comunicazione interattiva.
...
* I moduli AEM su un’istanza OSGi incorporati nei AEM Forms sul server JEE possono inoltre eseguire flussi di lavoro incentrati sui moduli in OSGi e nelle cartelle esaminate.
