...
* Quando il server AEM e la pagina Web si trovano in domini diversi, esegui i passaggi elencati nella sezione [Configurare gli URL di richiesta assoluti con GuideBridge](#configure-base-url) e [abilitare AEM Forms a distribuire i moduli adattivi a un sito tra domini diversi](#cross-site).
...
>[!TAB Utilizzo della configurazione baseUrl di GuideBridge]

Quando si utilizza la configurazione [GuideBridge `baseUrl`](#configure-base-url), configurare CORS nell&#39;istanza di pubblicazione di AEM in modo che il server AEM restituisca le intestazioni appropriate per gli endpoint di invio, precompilazione e documento di record.

1. Nell&#39;istanza di pubblicazione di AEM, passare a Gestione configurazione console Web AEM all&#39;indirizzo `https://'[server]:[port]'/system/console/configMgr`.
1. Individua e apri la configurazione **Adobe Granite Cross-Origin Resource Sharing Policy** (`com.adobe.granite.cors.impl.CORSPolicyImpl`).
1. Aggiungi l’origine della pagina web che incorpora alle origini consentite. Ad esempio, `https://www.example.com`.
1. Assicurati che i percorsi consentiti includano gli endpoint di AEM Forms utilizzati dai moduli adattivi incorporati, ad esempio gli URL di invio, precompilazione e documento record in `/content/forms/af/`.

>[!TAB Filtro referrer Apache Sling]
