...
* Quando il server AEM e la pagina web si trovano su domini diversi, esegui i passaggi elencati nella sezione [abilita AEM Forms per distribuire moduli adattivi a un sito tra più domini](#cross-site).
...
1. Nel codice incorporato:
   ...
   * Modifica il valore della variabile *options.path* con il percorso dell&#39;URL di pubblicazione del modulo adattivo. Se il server AEM è in esecuzione su un percorso contestuale, assicurati che l’URL includa il percorso contestuale. menziona sempre il nome completo del modulo adattivo, inclusa l’estensione.   Ad esempio, il codice e l&#39;adattivo di cui sopra risiedono sullo stesso server di AEM forms, quindi l&#39;esempio utilizza il percorso contestuale del modulo adattivo /content/forms/af/locbasic.html.
...
## Abilitare AEM Forms per distribuire moduli adattivi a siti con più domini {#cross-site}

1. Nell’istanza di pubblicazione di AEM, vai a AEM Web Console Configuration Manager all’indirizzo `https://'[server]:[port]'/system/console/configMgr`.
