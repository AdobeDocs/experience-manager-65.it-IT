...
seo-title: Installare Workbench
...
Le risorse riportate nella tabella sono utili per ottenere ulteriori informazioni e iniziare a utilizzare AEM Forms.
<table>
 <tbody>
  <tr>
   <td><p><strong>Per informazioni su</strong></p> </td>
   <td><p><strong>Vedi</strong></p> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><p><a href="http://adobe.com/go/learn_aemforms_introduction_65">Panoramica</a><br /> di AEM Forms <br /> </p> </td>
  </tr>
  <tr>
   <td><p>Tutta la documentazione disponibile per AEM Forms</p> </td>
   <td><p><a href="http://adobe.com/go/learn_aemforms_introduction_65">Documentazione</a><br /> su AEM Forms <br /> </p> </td>
  </tr>
  <tr>
   <td><p>Aggiornamenti patch, note tecniche e informazioni aggiuntive su questa versione del prodotto</p> </td>
   <td><p>Contattare il supporto</a><br /> Adobe Enterprise <br /> </p> </td>
  </tr>
 </tbody>
</table>
...
>[!NOTE]
>
>Flex Worksapce non è più supportato per i moduli AEM. È disponibile per la versione AEM Forms.

## Before You Install {#before-you-install}

### Panoramica sull&#39;installazione di Workbench {#workbench-installation-overview}

Workbench è un ambiente di sviluppo integrato (IDE) che sviluppatori e autori di moduli utilizzano per creare moduli e processi aziendali automatizzati. Viene inoltre utilizzata per gestire le risorse e i servizi utilizzati dai processi e dai moduli.
...
>[!NOTE]
>
>Il server AEM Forms richiede un programma di installazione a parte. Per ulteriori informazioni, consulta la documentazione di installazione di AEM Forms su JEE.
...
**Workbench** Sono consigliati almeno i seguenti requisiti:Spazio su disco per l&#39;installazione:
...
* Java™ Runtime Environment (JRE) 7.0 aggiorna 51 o versioni successive e aggiorna 7.0.
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>Acrobat Pro o Acrobat Pro Extended (compreso Designer)</p> </td>
   <td><p>Nessuno.<br /> 
...
È possibile creare diverse versioni di Designer sullo stesso sistema, ad esempio Designer 6.4.x per Workbench 6.4 e Designer 6.5.0.x per Workbench 6.5. Non è necessario disinstallare la versione di Designer installata con Acrobat 10 Pro o Acrobat 10 Pro Extended o versione successiva.
<br /></p> </td>
  </tr>
  <tr>
   <td><p>Designer (autonomo)</p> </td>
   <td><p>Nessuno. <br />La versione di Designer inclusa in Workbench è disponibile solo in lingua inglese. <br />Il programma di installazione di Workbench non reinstallerà una nuova versione di Designer. Viene invece corretta una versione aggiornata, inclusa nel pacchetto di installazione di Workbench. È inoltre possibile utilizzare la versione localizzata di Designer all'interno di Workbench.<br /> </p> </td>
  </tr>
 </tbody>
</table>
...
1. Vai al Pannello di **controllo > Programmi > Programmi e funzionalità**
1. Nell&#39;elenco Programmi attualmente installati, selezionare **Adobe Designer**.
1. Fate clic su **Disinstalla** , quindi fate clic su **Sì**.
...
1. Accettare la directory predefinita elencata oppure fare clic su Scegli e passare alla directory in cui si installa Workbench, quindi fare clic su Avanti.
...
1. I seguenti elementi sono ora installati nel computer:
   * **Workbench**: Per eseguire Workbench dal menu Start, selezionare Tutti i programmi > AEM Forms > Workbench, se si sceglie di archiviare la cartella dei collegamenti. Per ulteriori informazioni, vedere la documentazione <a href="https://helpx.adobe.com/content/dam/help/en/experience-manager/6-5/forms/pdf/WorkbenchHelp.pdf">Uso di Workbench</a> .
   * **Designer**: È possibile accedere a Designer dall&#39;interno di Workbench. Per ulteriori informazioni, vedere l&#39;argomento della Guida introduttiva nella Guida <a href="https://helpx.adobe.com/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf">di</a>Designer.
...
I moduli AEM su JEE possono essere aggiornati alle applicazioni AEM Forms tramite l&#39;Aggiornamento guidato. Per ulteriori informazioni, vedere Aggiornamento della documentazione sugli artifact legacy nella Guida di Workbench.

### Configurazione e accesso a un server {#configuring-and-logging-server}

Per utilizzare Workbench, è necessario che un&#39;istanza di AEM Forms sia in esecuzione, in genere su un computer a parte. Per accedere ad AEM Forms è necessario disporre di un nome utente e di una password, oltre a informazioni sulla posizione del server.

>[!NOTE]
>
>Se AEM Forms è stato configurato per l&#39;utilizzo del provider di repository EMC Documentum o IBM FileNet e si desidera accedere a un repository diverso da quello configurato come predefinito nella console di amministrazione dei moduli di AEM, fornire il nome utente username@Repository.

### Configurazione delle impostazioni di timeout {#configuring-timeout-settings}

Per impostazione predefinita, Workbench si chiude dopo due ore, indipendentemente dall&#39;attività o dall&#39;inattività. Per modificare l’impostazione relativa al timeout, consultate &quot;Configuring User Management > Configure advanced system attribute&quot; (Configurazione della gestione utente > Configura attributi di sistema avanzati) nella guida <a href="https://docs.adobe.com/content/help/en/experience-manager-65/forms/administrator-help/configure-user-management/configure-advanced-system-attributes.html">della console di</a>amministrazione.

### Configurazione di Workbench per la connessione mediante HTTPS {#configuring-workbench-to-connect-over-HTTPS}

Per connettere Workbench a un server AEM Forms mediante il protocollo HTTPS, è necessario assicurarsi che l&#39;autorità di certificazione (CA) che ha emesso la chiave pubblica sia riconosciuta come attendibile da Workbench. Se il certificato non viene riconosciuto come proveniente da un&#39;origine affidabile, è necessario aggiornare il file di registro che si trova nella directory [Workbench_HOME]/workbench/jre/lib/security.
...
1. Accertatevi di disporre di una copia del certificato SSL (Secure Sockets Layer). Contattare l&#39;amministratore che ha configurato il server SSL o esportare il certificato utilizzando un browser Web.
...
1. Aprire una finestra del prompt dei comandi, passare a [Workbench_HOME]/workbench/jre/bin, quindi digitare il comando seguente:
   `keytool -import -storepass changeit -file [Workbench_HOME]\workbench\jre\lib\security\ssl_cert_for_certname.cer -keystore [Workbench_HOME]\workbench\jre\lib\security\cacerts -alias example`Dove:
...
1. Quando viene richiesto di rendere affidabile il certificato, digitare Sì e premere il tasto Invio. Il keytool prosegue importando il file cacerts nella directory [Workbench_HOME]/workbench/jre/lib/security.
...
Il servizio Forms riceve i modelli in base al nome del file o alla posizione del repository, oppure in base al valore, come oggetto XML presente nella memoria.
...
* **Modelli passati per valore**: Utilizza i byte dei modelli passati insieme ai dati utilizzando il parametro theinDataDoc. Se si passano modelli univoci con UUID diverso, la cache del disco si espanderà all’infinito e non verrà mai riutilizzata. Per evitare questo problema, l&#39;attributo UUID deve essere rimosso da tutti i modelli per garantire che non venga creata alcuna cache per il modello. In alternativa, il passaggio dello stesso UUID non-null consente di creare gli oggetti della cache, ma assicura che la stessa cache venga aggiornata con ogni richiesta.

Per evitare che la cache cresca all&#39;infinito, è consigliabile considerare i seguenti fattori per il rendering dei modelli generati dinamicamente tramite le nuove API di AEM Forms, ovvero renderingHTMLForm2 e renderingPDFForm2.
...
* Se vengono passati più modelli di input passivated con lo stesso UUID e la stessa radice di contenuto, la stessa cache viene sovrascritta.
...
Le best practice seguenti garantiscono che la cache non si espanda all’infinito quando i modelli generati dinamicamente vengono passati al servizio Forms:
   * Eliminate l’UUID o passate lo stesso UUID in tutti i modelli generati dinamicamente.
   * Generare il documento dai byte dei modelli o dallo stesso nome file sul disco.

### Disinstallazione di Workbench {#uninstalling-workbench}

Utilizzare la funzione Installazione applicazioni nel Pannello di controllo per avviare il programma di disinstallazione. Le applicazioni Workbench e Designer dispongono di programmi di disinstallazione separati.

## Configurazione di AEM Forms XDC Editor {#configuring-aem-forms-xdc-editor}

Utilizzando XDC Editor, gli amministratori di rete possono creare e modificare i file XDC (XML Forms Architecture Device Configuration (XDC). I file XDC descrivono le caratteristiche delle stampanti, ad esempio il linguaggio di stampa o la correlazione tra il formato carta e la posizione del vassoio.
...
1. Nel server AEM Forms, individua la cartella XDC nella cartella principale [\sdk\samples\Output\IVS di]AEM Forms.
...
1. Andate al sito Web della documentazione di AEM Forms.
