...
* [Modelli](../../forms/using/creating-adaptive-form.md#create-an-adaptive-form-based-on-an-xfa-form-template)modulo Consente di selezionare un modello XFA per il modulo adattivo. Quando selezionate un modello XFA, potete utilizzare il file XDP associato per il documento del record come descritto sopra.

* [Schema](../../forms/using/creating-adaptive-form.md#create-an-adaptive-form-based-on-xml-or-json-schema)XML Consente di selezionare una definizione di schema XML per il modulo adattivo. Quando si seleziona uno schema XML per il modulo adattivo, è possibile:
...
**Modello XFA del modello** di record (file XDP) generato da un modulo adattivo.
...
* Alla pagina master con il nome `AF_MASTERPAGE` situato sotto il sottomodulo `AF_METATEMPLATE` principale viene data la preferenza per l&#39;estrazione di informazioni di intestazione, piè di pagina e stile.
...
1. Selezionate un pannello (pannello principale) nel documento del record, quindi toccate ![Configura](assets/configure.png).
