...
## Salva comunicazioni interattive come bozza {#save-as-draft}
...
1. Implementare l&#39;interfaccia [ccrDocumentInstance](https://helpx.adobe.com/experience-manager/6-5/forms/javadocs/com/adobe/fd/ccm/ccr/ccrDocumentInstance/api/services/CCRDocumentInstanceService.html) Service Provider Interface (SPI).
   ...
   Per un esempio di implementazione SPI, vedere [Esempio di implementazione di ccrDocumentInstance SPI](#sample-ccrDocumentInstance-spi).
1. Aprite `http://<hostname>:<port>/ system/console/bundles` e toccate **[!UICONTROL Installa/Aggiorna]** per caricare il bundle OSGi. Verificate che lo stato del pacchetto caricato sia visualizzato come **Active**. Riavviate il server se lo stato del pacchetto non viene visualizzato come **Active**.
1. Passa a `https://'[server]:[port]'/system/console/configMgr`.
1. Toccate **[!UICONTROL Crea configurazione corrispondenza]**.
1. Selezionare **[!UICONTROL Abilita salvataggio con CCRDocumentInstanceService]** e toccare **[!UICONTROL Salva]**.

### Salva una comunicazione interattiva come bozza {#save-as-draft-agent-ui}
...
Dopo aver salvato la comunicazione interattiva come bozza, toccate **[!UICONTROL Salva modifiche]** per salvare eventuali modifiche aggiuntive nella bozza.
...
[Per ] relatore si intende l’identificatore univoco per la versione bozza che viene generata dopo il salvataggio di una comunicazione interattiva come bozza.
...
Implementate l&#39; `ccrDocumentInstance` SPI per salvare una comunicazione interattiva come bozza. Esempio di implementazione dell&#39;SPI `ccrDocumentInstance`.
...
La tabella seguente illustra l&#39;implementazione `ccrDocumentInstance` SPI di esempio. Mostra come le operazioni `save`, `update`, `get` e `getAll` richiamano il servizio di database nell&#39;implementazione di esempio.
...
>[!NOTE]
>
>Assicurarsi di aggiornare la dipendenza `aemfd-client-sdk` a 6.0.160 nel file `pom.xml`.
