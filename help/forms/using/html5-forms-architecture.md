...
[Apache ](https://sling.apache.org/) Slingis incentrato sulle risorse. Utilizza un URL di richiesta per risolvere la risorsa. Ogni risorsa dispone di una proprietà **sling:resourceType** (o **sling:resourceSuperType**). In base a questa proprietà, al metodo di richiesta e alle proprietà dell’URL della richiesta, viene quindi selezionato uno script sling per gestire la richiesta. Questo script sling può essere un JSP o un servlet. Per i moduli HTML5, i nodi **Profile** fungono da risorse sling e **Profile Renderer** fungono da script sling che gestisce la richiesta di rendering del modulo mobile con un profilo particolare. Un **Modulo di rendering del profilo** è un JSP che legge i parametri da una richiesta e chiama il servizio Forms OSGi.

Per informazioni dettagliate sull&#39;endpoint REST e sui parametri di richiesta supportati, vedere [Modulo di rendering](/help/forms/using/rendering-form-template.md).
...
Quando un utente invia il modulo, il browser invia lo stato del modulo in formato JSON al [proxy del servizio ](../../forms/using/service-proxy.md); quindi il proxy del servizio di invio genera un XML di dati utilizzando i dati JSON e invia l&#39;XML di dati per inviare l&#39;endpoint.
...
**Adobe XFA Forms Renderer (com.adobe.livecycle.adobe-lc-forms-core)** è il nome visualizzato del bundle OSGi moduli HTML5 visualizzato dalla console di amministrazione Bundle View of Felix (https://[host]:[port]/system/console/bundle).
...
<table>
 <tbody>
  ...
  <tr>
   <td>Nessuno</td>
   <td>Non inserire nella cache artefatti<br /> </td>
  </tr>
  <tr>
   <td>Conservatore</td>
...
Per aggiornare queste impostazioni, accedete all&#39;Admin Console  CQ Felix (disponibile all&#39;indirizzo https://&lt;&#39;[server]:[porta]&#39;/system/console/configMgr), cercate e selezionate Configurazione Forms mobile.

È possibile configurare la dimensione della cache o disabilitare la cache utilizzando il servizio di configurazione. Potete inoltre abilitare il debug utilizzando il parametro Opzioni di debug. Per ulteriori informazioni sul debug dei moduli, vedere [Debug di moduli HTML5](/help/forms/using/debug.md).
...
Questo motore di scripting utilizza alcune delle funzioni di ECMAScript5 come Object.defineProperty. Il motore/libreria viene distribuito come CQ Client Lib con il nome della categoria **xfaforms.profile**. Fornisce inoltre **API FormBridge** per consentire l&#39;interazione con il modulo da parte di portali o app esterne. Con FormBridge, un&#39;app esterna può nascondere in modo programmatico alcuni elementi, ottenerne o impostarne i valori o modificarne gli attributi.
...
I profili sono i nodi delle risorse in sling che rappresentano una maschera o una famiglia di Forms. A livello di CQ, questi profili sono nodi JCR. I nodi risiedono nella cartella **/content** nell&#39;archivio JCR e possono trovarsi all&#39;interno di qualsiasi sottocartella all&#39;interno della cartella **/content**.

#### Modulo di rendering dei profili {#profile-renderers}

Il nodo Profilo ha una proprietà **sling:resourceSuperType** con valore **xfaforms/profile**. Questa proprietà invia internamente le richieste allo script sling per i nodi Profilo che si trovano nella cartella **/libs/xfaforms/profile**. Questi script sono pagine JSP, che sono contenitori per l&#39;assemblaggio dei moduli HTML e degli artefatti JS/CSS richiesti. Le pagine contengono riferimenti a:
...
Per ulteriori informazioni sulle librerie client CQ, vedere [Documentazione Clientlib CQ](https://docs.adobe.com/docs/en/cq/current/developing/components/clientlibs.html).
