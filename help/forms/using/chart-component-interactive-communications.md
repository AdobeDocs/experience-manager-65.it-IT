...
1. Trascina e rilascia la **Grafico** in uno dei seguenti componenti:

   * Canale di stampa: Area di destinazione o campo immagine
   * Canale Web: Pannello o area di destinazione

1. Tocca il componente grafico nell’editor delle comunicazioni interattive e seleziona **[!UICONTROL Configura (]** ![configure_icon](assets/configure_icon.png)) dalla barra degli strumenti Componente.
...
1. (Solo canale di stampa) Nel **[!UICONTROL Impostazioni agente]**, specifica se è obbligatorio per l&#39;agente utilizzare questo grafico. Se **[!UICONTROL È obbligatorio per l&#39;agente utilizzare questo grafico]** l’opzione non è selezionata, l’agente può toccare l’icona occhio del grafico nel **[!UICONTROL Contenuto]** scheda dell&#39;interfaccia utente dell&#39;agente per mostrare o nascondere il grafico.
...
<table>
 <tbody>
  ...
  <tr>
   <td>Suggerimento</td>
   <td><p>Formato in cui la descrizione comando viene visualizzata al passaggio del mouse su un punto dati del grafico nel canale Web. Il valore predefinito è ${x}(${y}). A seconda del tipo di grafico, quando si posiziona il mouse su un punto, una barra o una sezione del grafico, le variabili ${x}e ${y} vengono sostituite in modo dinamico con i valori corrispondenti sull'asse X e sull'asse Y e visualizzate nella descrizione comando.</p> <p>Per disattivare la descrizione comando, lascia <span class="uicontrol">Descrizione comandi</code> campo vuoto. Questa opzione non è applicabile ai grafici a linee e a superfici. Ad esempio, vedi <a href="#chartoutputprintweb">Esempio 1: Output grafico in stampa e web</a>.</code></p> </td>
   <td>Web</td>
  </tr>
  <tr>
   ...
   <td><p>Oltre alle configurazioni comuni, sono disponibili le seguenti configurazioni specifiche per il grafico:</p>
    <ul>
     ...
     <li><strong>Colore linea</strong>: È disponibile per i grafici a linee, a linee e a punti e Area per specificare il colore della linea nel grafico.</li>
     <li><strong>Colore punto</strong>: Disponibile per i grafici a punti e a linee e punti per specificare il colore dei punti del grafico.<br /> </li>
     <li><strong>Colore area</strong>: Disponibile per i grafici Area per specificare il colore dell’area sotto la linea del grafico.</li>
     <li><strong>Punto di riferimento &gt; Tipo di binding: </strong>Disponibile per grafici quadranti a<strong> </strong>specificare il tipo di binding per il punto di riferimento. Utilizzare la proprietà dell'oggetto modello dati o testo statico per definire il valore del punto di riferimento.</li>
     <li><strong>Punto di riferimento &gt; asse X: </strong>Disponibile per grafici Quadranti se si seleziona <span class="uicontrol">Statico</code> dall’elenco a discesa Tipo di binding per specificare il valore dell’asse X per il punto di riferimento.</code></li>
     <li><strong>Punto di riferimento &gt; Asse Y: </strong>Disponibile per grafici Quadranti se si seleziona <span class="uicontrol">Statico</code> dall’elenco a discesa Tipo di binding per specificare il valore dell’asse Y per il punto di riferimento.</code></li>
     <li><strong>Punto di riferimento &gt; Oggetto del modello dati per le serie: </strong>Disponibile per più grafici Quadranti serie se si seleziona <span class="uicontrol">Oggetto Data Model</code> dall’elenco a discesa Tipo di binding . Definisci le proprietà oggetto modello dati del modulo per identificare la serie per il punto di riferimento. </code></li>
...
Mentre il componente Grafico include alcune funzioni integrate, puoi scrivere [funzioni personalizzate](#customfunctionsweb) e renderli disponibili per l&#39;uso nella configurazione del grafico nel canale web.
...
1. Per visualizzare la funzione personalizzata nel menu a discesa Funzione, in CRXDe Lite, crea un `nt:unstructured` nella cartella apps con le seguenti proprietà:
   ...
   * Aggiungi proprietà `guideComponentType` con valore come `fd/af/reducer`. (mandatory)
...
* **[!UICONTROL Oggetti del modello dati]** - Selezionare le proprietà dell&#39;oggetto modello dati per creare binding dei dati per l&#39;asse X (Tipo di spesa) e l&#39;asse Y (Importo transazione).
...
### Grafico ad anello per web {#donutchartweb}
...
* **[!UICONTROL Oggetti del modello dati]** - Selezionare le proprietà dell&#39;oggetto modello dati per creare binding dei dati per l&#39;asse X (Tipo di spesa) e l&#39;asse Y (Importo transazione).
* **[!UICONTROL Raggio interno]** - Specificare il valore Raggio interno come 150 per specificare il raggio (in pixel) del cerchio interno nel grafico.
* **[!UICONTROL Descrizione comandi]** - Utilizzare il formato predefinito ${x}(${y}) per visualizzare la descrizione comando. La descrizione comando viene visualizzata come: Tipo di spesa (Importo transazione). Esempio: Addebito per Bitcoin(10000).
...
## Esempio 3: Grafico quadrante multiserie nel web {#example-multi-series-quadrant-chart-in-web}
...
* **Tipo grafico:** Seleziona **Quadrante** dall’elenco a discesa.

* Seleziona la **Serie multiple** casella di controllo.
* **Oggetto Data Model**: Specificare la proprietà dell&#39;oggetto modello dati per la serie. La proprietà dell&#39;oggetto modello dati per il nome della banca è un elemento padre delle proprietà dell&#39;oggetto modello dati tracciate nell&#39;asse X e nell&#39;asse Y.
...
* Seleziona la **Mostra legende** per visualizzare i codici colore dei nomi bancari.
