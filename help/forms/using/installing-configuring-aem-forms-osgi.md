...
**HTML5 Forms** è la versione intuitiva per il browser dei PDF forms. HTML5 Forms è adatto per ambienti che non supportano i plug-in PDF. HTML5 Forms consente il rendering di moduli basati su XFA su dispositivi mobili e browser desktop sui quali non è supportato PDF basato su XFA. Questi moduli sono particolarmente adatti per tablet e ambienti desktop.

 AEM Forms è una potente piattaforma di classe enterprise e l&#39;acquisizione dei dati (moduli adattivi, PDF forms e HTML5 Forms) è solo una delle funzionalità di  AEM Forms. Per un elenco completo delle funzionalità, consulta [Introduzione a  AEM Forms](/help/forms/using/introduction-aem-forms.md).
...
### (Solo per Windows) Installazione automatica dei redistribuibili di Visual Studio {#automatic-installation-visual-studio-redistributables}
...
Per valutare se i redistribuibili di Visual Studio sono installati automaticamente, aprire il `error.log` file disponibile nella `/crx-repository/logs/` directory. I file di registro includono il seguente messaggio:
...
1. Open the `[AEM installation directory]\crx-quickstart\conf\sling.properties` file for editing.
   ...
   Se avete usato `[AEM installation directory]\crx-quickstart\bin\start.bat` per avviare AEM, modificate le proprietà sling.properties che si trovano in `[AEM_root]\crx-quickstart\`.
...
Per utilizzare  Adobe Sign con  AEM Forms, [integrate  Adobe Sign con  AEM Forms](/help/forms/using/adobe-sign-integration-adaptive-forms.md).
