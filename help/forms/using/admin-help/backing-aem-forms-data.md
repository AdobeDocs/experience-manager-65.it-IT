...
Questa sezione descrive i passaggi necessari per completare un backup attivo o online del database dei moduli AEM, delle directory GDS e Content Storage Root.

Dopo che i moduli AEM sono stati installati e distribuiti nelle aree di produzione, l&#39;amministratore del database deve eseguire un backup iniziale completo o freddo del database. Il database deve essere chiuso per questo backup. Quindi, i backup differenziali o incrementali (o a caldo) del database dovrebbero essere eseguiti regolarmente.
...
## Eseguire il backup del database, del repository GDS, di AEM e delle directory principali di Content Storage {#back-up-the-database-gds-aem-repository-and-content-storage-root-directories}

I moduli AEM devono essere collocati in modalità di backup sicuro (snapshot) o di backup continuo (copertura continua). Prima di impostare i moduli AEM in modo da poter accedere a una delle modalità di backup, assicurarsi di disporre dei seguenti elementi:
...
* Se utilizzate backup in modalità di scorrimento o snapshot, accertatevi che il database sia configurato con le impostazioni di registro corrette per consentire il backup a caldo del database. (Vedere Database [moduli](/help/forms/using/admin-help/files-back-recover.md#aem-forms-database)AEM.)
...
* Esegui il backup delle istanze di creazione e pubblicazione (backup crx-repository).
   ...
   * La decisione relativa alla lunghezza di ciascuna sessione di backup (per la modalità di backup continuo) deve basarsi sul tempo totale impiegato per il backup di tutti i dati nei moduli AEM (DB, GDS, archivio AEM e qualsiasi altro dato personalizzato aggiuntivo).

È necessario eseguire il backup del database dei moduli AEM, compresi eventuali log delle transazioni. (Vedere Database [moduli](/help/forms/using/admin-help/files-back-recover.md#aem-forms-database)AEM.) Per ulteriori informazioni, consultate l&#39;articolo appropriato della knowledge base per il database:
...
Questi articoli forniscono indicazioni sulle funzionalità di base del database per il backup e il ripristino dei dati. Non sono concepite come guide tecniche complete per una specifica funzionalità di backup e ripristino del database di un fornitore. Illustra i comandi necessari per creare una strategia di backup affidabile del database per i dati dell&#39;applicazione dei moduli AEM.
...
1. Selezionate Opera in modalità di backup sicuro e fate clic su OK.
   ...
   Questo metodo consente ai moduli AEM di passare alla modalità di backup indefinitamente (senza timeout) e di passare alla modalità snapshot anziché alla modalità di backup a rotazione.

**Utilizzo dell&#39;opzione della riga di comando per passare alla modalità di backup sicura**

È possibile utilizzare gli script di interfaccia della riga di comando `LCBackupMode` per posizionare i moduli AEM in modalità di backup sicuro.
...
1. Al prompt dei comandi, eseguire il comando seguente su una singola riga:
   ...
   * (Windows) `LCBackupMode.cmd enter [-Host=`*hostname *`] [-port=`*portnumber* `] [-user=`*nomeutente *`] [-password=`*password* labelname `] [-label=`**`] [-timeout=`** secondi `]`
   * (Linux, UNIX) `LCBackupMode.sh enter [-host=`*hostname *`] [-port=`*portnumber* `] [-user=`*nomeutente *`] [-password=`*password* `] [-label=`*labelname *`]`
   ...
   `Host` è il nome dell&#39;host in cui sono in esecuzione i moduli AEM.

   `port` è la porta WebServices del server applicazioni su cui sono in esecuzione i moduli AEM.

   `user` è il nome utente dell’amministratore dei moduli di AEM.

   `password` è la password dell’amministratore di moduli AEM.
...
Per utilizzare la console di amministrazione per escludere i moduli AEM dalla modalità di backup sicuro (modalità snapshot), effettuate le seguenti operazioni.

1. Accedi alla console di amministrazione.
1. Fate clic su Impostazioni > Impostazioni di sistema di base > Utility di backup.
1. Deselezionare Opera in modalità di backup sicuro e fare clic su OK.

**Tutte le modalità di backup**

Potete utilizzare l&#39;interfaccia della riga di comando per escludere i moduli AEM dalla modalità di backup sicura (modalità snapshot) o per terminare la sessione corrente della modalità di backup (modalità rollmode). Non è possibile utilizzare la console di amministrazione per uscire dalla modalità di backup continuo. In modalità di backup continuo, i controlli Utilità di backup nella console di amministrazione sono disabilitati. È necessario utilizzare una chiamata API o il comando LCBackupMode.
...
1. Eseguire il comando seguente su una sola riga:
   ...
   * (Linux, UNIX) `LCBackupMode.sh leaveContinuousCoverage [-Host=`*nomehost *`] [-port=`*portnumber* `] [-user=`*nomeutente *`] [-password=`*password*`]`
      ...
      `port` è la porta su cui i moduli AEM sono in esecuzione sul server delle applicazioni.
