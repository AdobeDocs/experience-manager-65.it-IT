...
description: Il servizio Output memorizza nella cache le strutture del modulo, i frammenti e le immagini. Scoprite come configurare la cache per l'output.
seo-description: Il servizio Output memorizza nella cache le strutture del modulo, i frammenti e le immagini. Scoprite come configurare la cache per l'output.
...
# Configurazione della memorizzazione nella cache per Output {#configuring-caching-for-output}
...
È possibile specificare le impostazioni utilizzate da Output per il caching, che possono ottimizzare le prestazioni dell&#39;ambiente dei moduli AEM.
...
**** Cache di rendering del modulo abilitata: Per impostazione predefinita, questa opzione è selezionata, il che significa che i moduli sottoposti a rendering vengono memorizzati nella cache per il successivo recupero. Questa impostazione ha poco effetto sulle prestazioni del servizio Output perché non memorizza nella cache i documenti non interattivi. Questa opzione ha effetto quando si utilizza il servizio Output per documenti non interattivi di cui viene eseguito il rendering sul client.
...
**** Abilitata per frammenti e immagini: Il servizio Output memorizza nella cache frammenti e immagini. Questa è l&#39;opzione predefinita.
