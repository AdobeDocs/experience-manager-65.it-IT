...
description: Fornisce informazioni sugli eventi community in formato calendario
...
e trascinarlo nella posizione desiderata su una pagina, ad esempio una posizione relativa alla caratteristica da rivedere.

Per informazioni necessarie, visitare il sito [Nozioni di base sui componenti community](/help/communities/basics.md).

Quando [librerie lato client richieste](/help/communities/calendar-basics-for-developers.md#essentials-for-client-side) sono inclusi, è così che `Calendar` verrà visualizzato.

![calendar-component](assets/calendar-component.png)

### Configurazione del calendario {#configuring-calendar}

Seleziona la inserita `Calendar` per accedere e selezionare il `Configure` che apre la finestra di dialogo per modifica.
...
Sotto **Impostazioni** , specificare se consentire o meno l&#39;applicazione di tag alle voci del calendario.
...
* **Moderato**
   ...
   Se questa opzione è selezionata, è necessario approvare la pubblicazione di eventi e commenti del calendario prima che vengano visualizzati in un sito pubblicato. L&#39;impostazione predefinita è deselezionata.
...
* **Tipi di immagine di copertina consentiti**
   ...
   Un elenco separato da virgole di estensioni di file di immagine con il separatore &quot;punto&quot;. Il valore predefinito è `.jpg,.jpeg,.png,.gif,.bmp`.
...
* **Consenti agli utenti di eliminare commenti ed eventi**
   ...
   Se questa opzione è selezionata, consentire ai membri di eliminare i commenti e gli eventi calendario pubblicati. Il valore predefinito è** **selezionato.
...
* **Rifiuta post**
   ...
   Se questa opzione è selezionata, i moderatori membri di fiducia potranno rifiutare i post e impedirne la visualizzazione nel forum pubblico. Il valore predefinito è selezionato.
...
>[!NOTE]
>
>Visita [Amministrazione dei tag](/help/sites-administering/tags.md) per scoprire come aggiungere un nuovo spazio dei nomi tag (tassonomia).
...
Nell’ambiente di pubblicazione, la funzione calendario visualizzerà un campo di ricerca con un intervallo di date predefinito ed eventuali eventi di calendario che rientrano in tale intervallo.

Quando si seleziona un evento calendario, vengono visualizzati i dettagli, la descrizione e i commenti relativi.
