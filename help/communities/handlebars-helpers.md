...
Un supporto per blocchi che verifica il valore corrente di [Modalità WCM](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) in un elenco di modalità separate da stringa.
...
* **contesto**: Stringa o oggetto
   ...
   Utilizzo `this` per passare il contesto corrente.

   Utilizzo `this.id` per ottenere la risorsa in `id` per il rendering del resourceType richiesto.
...
* Una chiamata helper Handlebars è un identificatore semplice ( *name* dell&#39;helper), seguita da zero o più parametri separati da spazio.
