...
* **Estensibile**: L&#39;implementazione dei componenti è orientata agli oggetti e si basa sulla logica aziendale: è facile aggiungere un accesso incrementale al business sul server
...
* Aggiunta di una nuova operazione personalizzata
   ...
   * Usa [OperationServices](server-customize.md#operationservice-class) esistente come necessario
...
1. Trasmette la risorsa (resourceType) al gruppo `SocialComponentFactoryManager`e riceve un SocialComponentFactory in grado di selezionare una risorsa `SocialComponent`che rappresenta la risorsa.
...
Per informazioni sulla gestione di UGC memorizzati nell&#39;archivio [dei contenuti della](working-with-srp.md)community, vedete
