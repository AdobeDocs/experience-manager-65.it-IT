...
* Installate gli ultimi aggiornamenti [di](#aem64updates)AEM 6.5.

* Se non utilizzate le porte predefinite (4502, 4503), [configurate gli agenti](#replication-agents-on-author)di replica.
...
* Identificare la scelta per lo storage [UGC](/help/communities/working-with-srp.md) condiviso (**SRP**)
   ...
   * Se JCR SRP [(JSRP)](/help/communities/jsrp.md)
      ...
      * Store UGC non condiviso:
         ...
         * UGC è visibile solo sull&#39;istanza o sul cluster AEM cui è stato immesso.
...
AEM 6.5 Communities GA include il pacchetto Community. Per informazioni sugli aggiornamenti di AEM 6.5 [Communities](/help/release-notes/release-notes.md#experiencemanagercommunities), consulta [AEM Note](/help/release-notes/release-notes.md#communities-release-notes.html)sulla versione 6.5.
...
1. Se l&#39;installazione avviene su una distribuzione esistente dopo la configurazione di JDBC, eseguire un nuovo riferimento JDBC al nuovo connettore salvando nuovamente la configurazione JDBC dalla console Web:
   ...
   * Individuate `Day Commons JDBC Connections Pool` la configurazione e selezionate per aprirla.
   * Seleziona `Save`.

1. Ripetete i passaggi 3 e 4 per tutte le istanze di creazione e pubblicazione.
...
Per impostazione predefinita, la configurazione `AEM Communities Publisher Configuration` OSGi è configurata con la casella di **`Primary Publisher`** controllo selezionata, in modo che tutte le istanze di pubblicazione in una farm di pubblicazione si identifichino automaticamente come principali.

Pertanto, è necessario **modificare la configurazione in tutte le istanze** di pubblicazione secondarie per deselezionare la **`Primary Publisher`** casella di controllo.
...
* Deselezionare la casella di controllo Editore **** principale
* Seleziona **Salva**
...
* Se l&#39;editore non è localhost:4503 o l&#39;utente di trasporto non lo è, `admin`configurare l&#39;agente [](#replication-agents-on-author)di replica.
...
* Selezionate l’icona di modifica
* Selezionare la casella di controllo **attivazione**
* select **Save**
...
* Accedere all&#39;istanza AEM, in genere un&#39;istanza di creazione, che contiene il materiale chiave da copiare
   ...
   * Copiate i file hmac e i file dei nodi principali.
