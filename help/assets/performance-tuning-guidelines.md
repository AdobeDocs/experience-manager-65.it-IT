...
Una volta pronto il volume temporaneo ad alte prestazioni, impostare il parametro JVM `-Djava.io.tmpdir`. Ad esempio, potete aggiungere il parametro JVM di seguito alla `CQ_JVM_OPTS` variabile nello `bin/start` script di [!DNLEExperience Manager]:
...
La funzione di riserva XMP aggiorna la risorsa originale ogni volta che i metadati vengono modificati in [!DNL Experience Manager], determinando le seguenti situazioni:
...
Se gli utenti non devono effettuare la ricerca full-text delle risorse, ad esempio, cercando testo nei documenti PDF, disattivatela. È possibile migliorare le prestazioni dell&#39;indice disattivando l&#39;indicizzazione full-text. Per disattivare l’estrazione [!DNL Apache Lucene] del testo, effettuate le seguenti operazioni:
