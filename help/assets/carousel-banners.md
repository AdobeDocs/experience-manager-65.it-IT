...
1. [Aggiunta di punti attivi o mappe immagine a un banner immagine](#adding-hotspots-or-image-maps-to-an-image-banner).
   ...
   Consulta [(Facoltativo) Anteprima dei banner a carosello](#optional-previewing-carousel-banners) - Facoltativo. Se lo desideri, puoi visualizzare una rappresentazione del set carosello e testarne l’interattività.

1. [Pubblicare i banner a carosello](#publishing-carousel-banners).

   Pubblichi un set carosello come faresti con una risorsa. In Assets, passa al set carosello, selezionalo e fai clic su **[!UICONTROL Pubblica]**. La pubblicazione di un set carosello attiva l’URL e la stringa di incorporamento.
...
1. Effettua una delle operazioni seguenti:
   ...
   * [Aggiungi un banner carosello al tuo sito web nell’Experience Manager](/help/assets/adding-dynamic-media-assets-to-pages.md) Se sei un cliente di Experience Manager Sites, puoi aggiungere il set carosello direttamente alla pagina in questo Experience Manager, utilizzando il componente File multimediali interattivi.

Per modificare i set carosello, consulta [modifica di set carosello](#editing-carousel-sets). Inoltre, è possibile visualizzare e modificare [Proprietà set carosello](manage-assets.md#editing-properties).
 ...
 <tbody>
  <tr>
   ...
   <td><p>Gli URL registrati di Quickview includono:</p>
    ...
    </ul> <p>L’unica parte variabile nell’URL è il valore della proprietà <code>productId=</code> parametro stringa query ed è chiaramente un valore SKU. Pertanto, i punti attivi o le mappe immagine richiedono solo campi SKU compilati con valori come <code>866558,</code> <code>1196184,</code> <code>1081492,</code> <code>1898294.</code></p> </td>
  </tr>
  <tr>
...
Se hai già caricato le immagini che desideri utilizzare, passa al passaggio successivo, [Creare set carosello](#creating-carousel-sets). Tieni presente che le immagini utilizzate nel carosello devono essere caricate dopo l’abilitazione di Dynamic Medie.
...
1. In Assets, individua la cartella in cui vuoi creare il set carosello e passa a **[!UICONTROL Crea]** > **[!UICONTROL Set carosello]**.
1. Nella pagina dell’editor di banner a carosello, seleziona **[!UICONTROL Toccate per aprire il selettore risorse]** per selezionare l&#39;immagine per la prima diapositiva.
...
1. (Facoltativo) Effettuate una delle seguenti operazioni:
   ...
   * Per eliminare un’immagine, selezionala, quindi seleziona **[!UICONTROL Elimina diapositiva]** sulla barra degli strumenti.
...
1. Puoi modificare la progettazione visiva e il comportamento dei set carosello. Seleziona la **[!UICONTROL Comportamento]** e **[!UICONTROL Aspetto]** e regolare la modalità di visualizzazione del banner del carosello o il comportamento di componenti specifici. Consulta [Gestire i predefiniti visualizzatore](/help/assets/viewer-presets.md) per ulteriori informazioni su come utilizzare l’editor visualizzatore.
   ...
   >
   >Consulta [Considerazioni speciali per i banner a carosello](/help/assets/managing-viewer-presets.md#special-considerations-for-creating-a-carousel-banner-viewer-preset) per istruzioni dettagliate e ulteriori informazioni sull’editor visualizzatore.

   Puoi anche visualizzare in anteprima come verrà visualizzato il banner del carosello. Consulta [(Facoltativo) Anteprima dei banner a carosello](#optional-previewing-carousel-banners).

1. Seleziona **[!UICONTROL Salva]** al termine.
...
1. Nel campo di testo Nome digitare il nome del punto attivo o della mappa immagine. Questo nome viene visualizzato anche nel **[!UICONTROL Mappe e punti attivi]** elenco a discesa. Specificando un nome è facile identificare il punto attivo o la mappa immagine se si decide di modificarlo in futuro.
1. Effettua una delle seguenti operazioni in **[!UICONTROL Azioni]** scheda:
   ...
   * Seleziona **[!UICONTROL Quickview]**.
      ...
      * Se non sei un cliente Experience Manager Sites o e-commerce
         ...
         * Consulta [Identificare le variabili dei punti attivi](#identifying-hotspot-and-image-map-variables) se desideri definire queste variabili.
   ...
   * Seleziona **[!UICONTROL Collegamento ipertestuale]**.
   ...
   Consulta [Utilizzo dei selettori](/help/assets/working-with-selectors.md) per ulteriori informazioni.
...
1. Effettua una delle seguenti operazioni:
   ...
   * Passa il puntatore del mouse su una risorsa set carosello, quindi seleziona **[!UICONTROL Modifica]** (icona della matita).
   * Passa il puntatore del mouse su una risorsa set carosello, seleziona **[!UICONTROL Seleziona]** (icona di spunta), quindi seleziona **[!UICONTROL Modifica]** sulla barra degli strumenti.

   * Seleziona una risorsa Set carosello, quindi nell’angolo superiore sinistro della pagina seleziona **[!UICONTROL Modifica]** (icona della matita).

1. Per modificare il set carosello, effettuate una delle seguenti operazioni:

   * Per aggiungere una diapositiva, selezionare **[!UICONTROL Aggiungi diapositiva]** , quindi individua la risorsa da aggiungere alla diapositiva e seleziona il segno di spunta.
   ...
   * Per modificare l’aspetto o il comportamento del set carosello, seleziona la **[!UICONTROL Aspetto]** scheda o **[!UICONTROL Comportamento]** , quindi impostare le opzioni desiderate.
   * Per modificare punti attivi o mappe immagine, nella diapositiva appropriata, seleziona un punto attivo o una mappa immagine e cambia come necessario sotto **[!UICONTROL Azioni]** scheda.
   * Per eliminare una diapositiva, selezionarla, quindi selezionare **[!UICONTROL Elimina diapositiva]** sulla barra degli strumenti.
   * Per applicare un predefinito, seleziona la voce corrispondente nell’angolo superiore destro della pagina **[!UICONTROL Predefinito]** , quindi seleziona un predefinito visualizzatore.
...
Consulta [Collegamento di URL all’applicazione web](/help/assets/linking-urls-to-yourwebapplication.md). Il metodo di collegamento basato su URL non è possibile se il contenuto interattivo presenta collegamenti con URL relativi, in particolare collegamenti a pagine Experience Manager Sites.
Consulta [Aggiunta di risorse Dynamic Medie alle pagine](/help/assets/adding-dynamic-media-assets-to-pages.md).
...
1. In entrata **[!UICONTROL Risorse]**, passa a un banner carosello esistente creato e seleziona per aprirlo.
...
1. In **[!UICONTROL Visualizzatori]** nel pannello a sinistra della pagina, seleziona il nome del predefinito visualizzatore banner per carosello che desideri utilizzare.
1. Seleziona gli hotspot o le mappe immagine sull&#39;immagine in modo da poter testare le azioni associate.
...
Consulta [Pubblicare risorse Dynamic Medie](/help/assets/publishing-dynamicmedia-assets.md) per informazioni su come pubblicare i banner a carosello.
