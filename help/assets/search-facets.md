...
1. Dalla pagina Moduli di ricerca, seleziona Barra **[!UICONTROL di ricerca Amministratore]** risorse, quindi tocca **Modifica** ![aemassets_edit](assets/aemassets_edit.png).
   ...
   >1. Dal percorso */libs/settings/dam/search/facets/assets/jcr:content/items*, copiate la **risorsa, la directory, il tipo, i percorsi** esclusi e il tipo di **ricerca** dei nodi nel percorso indicato al punto 1.
   >1. Salva le modifiche.


1. Nella pagina Modifica moduli di ricerca, trascinare un predicato dalla scheda **[!UICONTROL Seleziona predicato]** al riquadro principale. Ad esempio, trascinare Predicato **[!UICONTROL proprietà]**.
...
1. Nel campo Nome **** proprietà, specificate un nome valido per la proprietà di metadati che desiderate associare al predicato. È il nome in base al quale viene eseguita la ricerca. Ad esempio, immettere `jcr:content/metadata/dc:description` o `./jcr:content/metadata/dc:description`.
...
Ad esempio, se selezionate &quot;Immagini&quot; per i tipi di file, la query di ricerca per le risorse viene creata combinando la `value` proprietà utilizzando un&#39;operazione OR. Ad esempio, la query di ricerca per le immagini è realizzata combinando i risultati associati per *image/jpeg*, *image/gif*, *image/png*, *image/pjpeg* e *image/tiff* `jcr:content/metadata/dc:format` per la proprietà tramite un&#39;operazione OR.

![La proprietà Value di un tipo di file, come visualizzata in CRXDE, viene utilizzata per il funzionamento delle query di ricerca](assets/chlimage_1-418.png)
...
Invece di creare manualmente una struttura di nodi per le opzioni nell&#39;archivio CRXDE, potete definire le opzioni in un file JSON specificando le coppie chiave-valore corrispondenti. Specificate il percorso del file JSON nel campo Nome **** proprietà. Ad esempio, puoi definire le coppie chiave-valore, `image/bmp`, `image/gif`, `image/jpeg`e `image/png` specificarne i valori come mostrato nel seguente file JSON di esempio. Nel campo Nome **** proprietà potete specificare il percorso CRXDE per il file.
...
1. Toccate il logo AEM, quindi passate a **[!UICONTROL Strumenti > Generale > Cerca moduli]**.
1. Dalla pagina **[!UICONTROL Moduli]** di ricerca, seleziona Barra **[!UICONTROL di ricerca Amministratore]** risorse, quindi tocca l’icona Modifica.
1. Nella pagina **[!UICONTROL Modifica modulo]** di ricerca, trascinare **[!UICONTROL Opzioni predicato]** dalla scheda **[!UICONTROL Seleziona predicato]** al riquadro principale.
1. Nella scheda **[!UICONTROL Impostazioni]** , immettere un&#39;etichetta e un nome per la proprietà. Ad esempio, per cercare le risorse in base al loro formato, specificate un nome descrittivo per l’etichetta, ad esempio Tipo **** file. Specificare la proprietà in base alla quale eseguire la ricerca nel campo della proprietà, ad esempio `jcr:content/metadata/dc:format.`
1. Effettua una delle operazioni seguenti:

   * Nel campo Nome **** proprietà, menzionare il percorso del file JSON in cui si definiscono i nodi per le opzioni e si specificano le coppie chiave-valore corrispondenti.
   * Toccate o fate clic sul `+` simbolo accanto al campo Opzioni per specificare il testo e il valore di visualizzazione per le opzioni che desiderate fornire nel pannello Filtri. Per aggiungere un’altra opzione, toccate o fate clic sul `+` simbolo e ripetete il passaggio.

1. Assicurarsi che l&#39;opzione Selezione **** singola sia deselezionata per consentire all&#39;utente di selezionare più opzioni per i tipi di file alla volta (ad esempio, Immagini, Documenti, Multimedia e Archivi). Se selezionate Selezione **** singola, l&#39;utente può selezionare una sola opzione per i tipi di file alla volta.
...
1. Fate clic sul logo AEM, quindi andate a **[!UICONTROL Strumenti]** > **[!UICONTROL Generale]** > **[!UICONTROL Moduli]** di ricerca.
1. Nella pagina Moduli di ricerca, seleziona Barra **[!UICONTROL di ricerca Amministratore]** risorse e tocca **[!UICONTROL Modifica]** ![aemassets_edit](assets/aemassets_edit.png).
1. Nella pagina Modifica modulo di ricerca, trascinare un predicato **[!UICONTROL proprietà]** multivalore dalla scheda **[!UICONTROL Seleziona predicato]** al riquadro principale.
1. Nella scheda **[!UICONTROL Impostazioni]** , inserite un&#39;etichetta e un testo segnaposto per il predicato. Specificare il nome della proprietà in base al quale eseguire la ricerca, ad esempio nel campo della proprietà `jcr:content/metadata/dc:value`. È inoltre possibile utilizzare la finestra di dialogo di selezione per selezionare un nodo.
1. Assicurarsi che **[!UICONTROL Delimiter Support]** sia selezionato. Nel campo **[!UICONTROL Delimitatori]** di input, specificate i delimitatori per separare i singoli valori. Per impostazione predefinita, la virgola è specificata come delimitatore. È possibile specificare un delimitatore diverso.
1. Nel campo **Descrizione** , immettete una descrizione facoltativa e toccate **[!UICONTROL Fine]**.
1. Andate al pannello Filtri nell’interfaccia utente Risorse. Al pannello viene aggiunto il **[!UICONTROL predicato Proprietà]** ValoreMulti.
...
1. Fate clic sul logo AEM, quindi andate a **[!UICONTROL Strumenti]** > **[!UICONTROL Generale]** > **[!UICONTROL Moduli]** di ricerca.
1. Dalla pagina Moduli di ricerca, seleziona Barra di ricerca **[!UICONTROL Amministratore]** risorse, quindi tocca **[!UICONTROL Modifica]** ![aemassets_edit](assets/aemassets_edit.png).
...
1. Nella scheda Impostazioni, inserite un testo segnaposto per il predicato. Specificate il nome della proprietà in base al quale eseguire la ricerca nel campo delle proprietà, ad esempio *jcr:content/metadata/cq:tags*. In alternativa, è possibile selezionare un nodo in CRXDE dalla finestra di dialogo di selezione.
1. Configurate la proprietà del percorso dei tag radice di questo predicate per popolare i vari tag nell&#39;elenco Tag.
1. Selezionate l’opzione **[!UICONTROL Mostra tutti i tag per]** cercare le risorse che includono tutti i tag specificati.
...
1. Selezionate **[!UICONTROL Corrispondenza con tutto]** per cercare corrispondenze che includono tutti i tag specificati.
...
| [!UICONTROL Dimensione file] | Cercare un predicato per cercare le risorse in base alla loro dimensione. Si tratta di un predicato basato su silder in cui potete selezionare le opzioni del cursore da un nodo configurabile. Le opzioni predefinite sono definite in /libs/dam/options/predicates/filesize nel repository CRXDE. La dimensione del file è specificata in byte. | <ul><li>Etichetta</li><li>Nome proprietà</li><li>Percorso</li><li>Descrizione</li></ul> |
...
1. Toccate **[!UICONTROL Elimina]** nella barra degli strumenti.
...
1. Nella finestra di dialogo di conferma, toccate **[!UICONTROL Elimina]** per rimuovere le modifiche personalizzate.
   ...
   Dopo aver eliminato le modifiche personalizzate ai facet di ricerca, l&#39;icona Blocca viene visualizzata nuovamente prima della Barra **[!UICONTROL di ricerca Amministratore]** risorse nella pagina Moduli **[!UICONTROL di]** ricerca.
